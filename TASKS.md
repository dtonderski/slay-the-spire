...
Status: in progress. Initial seam extraction is complete: captured seed-start Neow branch tables now live behind a first-class `sts_core::run::neow` facade, with focused tests preserving VERIFY01/CODEX03/CODEX04/TEST/M290001/M290008 behavior. Source-backed Ironclad A0 option generation is now implemented behind that facade: target jar bytecode pins `NeowEvent.rng = Random(Settings.seed)`, the slot order, slot reward/drawback pools, and the five option-screen RNG draws. Seed-start branch dispatch now uses the generated selected option reward for transform, Neow's Lament, colorless-card, common-relic, and simple immediate branches rather than seed-name branch routing, while retaining captured side effects where reward application is not generalized yet. Normal `THREE_CARDS` generation is source-backed as common/uncommon only with target `0.33f` uncommon rolls on `NeowEvent.rng`; forced-rare card reward generation for `ONE_RANDOM_RARE_CARD` and `THREE_RARE_CARDS` is source-backed on `NeowEvent.rng` with explicit counter tests. Colorless reward generation for `RANDOM_COLORLESS` and `RANDOM_COLORLESS_2` is source-backed as a split-stream helper: rarity burns on `NeowEvent.rng`, identity draws on `cardRng`, and duplicate retries consume only `cardRng`; CODEX04/TEST seed-start colorless reward choices now use this generator, and rare-colorless verifier routing is covered by synthetic/helper tests. Neow's three-potion reward is source-backed as three direct full-pool `potionRng` draws, not reward-rarity potion logic. Fixed-tier Neow common/rare relic rewards now initialize relic pools and pop the requested tier without a relic-tier roll; VERIFY01's Toy Ornithopter identity and simple-drawback rare relic helper coverage now use this path in seed-start verification. No-RNG immediate Neow gold/max-HP rewards and gold/max-HP/HP-loss drawbacks are implemented as core helpers. Neow-specific remove-one/remove-two/upgrade grids now open and confirm without shop cost or rest-phase side effects. Boss-swap application removes Burning Blood before popping the initialized boss relic pool, preserving relic-pool initialization as the only relic RNG consumption; current verifier follow-up slices cover Calling Bell queued relic rewards, Astrolabe, Pandora's Box, Empty Cage, and Tiny House reward-screen opening with skip/card-pick follow-ups, and the current Ironclad A0 boss relic pool has no known unsupported initial boss-swap outcomes. Neow transform generation now consumes `NeowEvent.rng` over the shared Ironclad transform pool and returns base cards; M290001/M290008 transform identity is generated and the old captured transformed-card lookup is retired, while Astrolabe still applies its separate upgrade step. Seed-start verification now supports the source-possible `Curse + TransformTwoCards` branch through a generated curse, transform-two grid, and synthetic CommunicationMod-style prefix. Remaining verifier application stays caveated for branch combinations and trace coverage that have only helper/synthetic evidence, including selected-trace exercise of remove-two/upgrade grids, rare-card/rare-colorless/rare-relic/potion branches, supported curse combos, and boss-swap follow-ups; impossible curse plus remove/upgrade/potion/boss-swap combos are not milestone gaps.
