...
public class CommunicationMod implements PostInitializeSubscriber, PostUpdateSubscriber, PostDungeonUpdateSubscriber, PreUpdateSubscriber, OnStateChangeSubscriber {
    ...
    /**
     * Sends persistent profile inputs as an explicit pre-run protocol record.
     * The collector stores this record in trace metadata; it is never derived
     * from a later game-state observation.
     */
    public static void sendProfileSnapshot() {
        ...
        if (CardCrawlGame.playerPref != null) {
            ...
            if (noteCard != null && !noteCard.trim().isEmpty()) {
                values.addProperty("note_card", noteCard);
            }
            values.addProperty(
                    "note_upgrades",
