...
- Combat deck shuffle validation: `CardGroup.shuffle` calls Java `Collections.shuffle` with `new java.util.Random(shuffleRng.randomLong())`, not direct RandomXS128 bounded draws. `CardGroup.initializeDeck` starts from the current master-deck order, adds non-innate/non-bottled cards to the draw pile, then adds innate/bottled cards to the top. CommunicationMod lists draw/discard piles bottom-first; the simulator draws from the pile top (last array entry). This reproduces selected Ironclad A0 first-combat opening piles for starter-only decks and modified decks with Neow colorless additions or transforms (CODEX04, TEST, M290001, M290008).
