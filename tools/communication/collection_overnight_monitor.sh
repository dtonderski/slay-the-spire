...
restart_game() {
  ...
  nohup env \
    ...
    PATH="$PATH" \
    "$NODE" "$ROOT/tools/communication/random_fidelity_game_watchdog.js" \
...
restart_campaign() {
  ...
  nohup env \
    ...
    STS_VERIFY_BIN="$ROOT/simulator/target/release/sts_verify" \
    STS_RANDOM_GAME_SEED_PREFIX=FIDL \
    STS_RANDOM_SOURCE_VERSION="$SOURCE_VERSION" \
    PATH="$PATH" \
    "$NODE" "$ROOT/tools/communication/run_random_fidelity_campaign.js" \
