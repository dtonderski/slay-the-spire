...
- `run_guided_collect.cmd` starts the SlayTheData-guided headless collector.
  ...
  `python\target\guided-collect\latest.json`, archives timestamped attempts under
  `python\target\guided-collect\reports\`, and forwards any extra
...
- `trace_tools.js validate <trace.jsonl>` checks that every action has a following state or error row for the same step and prints seeds, starts, rooms, encounters, deaths, terminal state, elite/boss room coverage, TCP `command_accept` / observed-timeout counts, and a simple harvest score.
