...
function summarize(message) {
  ...
  const summary = {
    ...
    gold: gs.gold ?? null,
    playtime_seconds: gs.playtime_seconds ?? null,
...
function writeAction(command, commandMeta) {
  ...
  if (commandMeta) {
    ...
    const playtimeSeconds = commandMeta.metadata?.playtime_seconds;
    if (Number.isSafeInteger(playtimeSeconds) && playtimeSeconds >= 0) {
      actionRecord.playtime_seconds = playtimeSeconds;
    }
  }
...
writeRecord({
  ...
  client: "simulator/tools/communication/trace_client.js",
  client_pid: clientPid,
