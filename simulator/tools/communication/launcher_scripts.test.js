...
function testRunAutoCollectDelegatesAndPreservesExitCode() {
  ...
  assert.match(script, /call "%REPO%\\simulator\\tools\\communication\\run_guided_collect\.cmd" %\*/);
  assert.match(script, /set RESULT=%ERRORLEVEL%/);
  assert.match(script, /call "%REPO%\\simulator\\tools\\communication\\run_guided_collect_status\.cmd"/);
...
function testGuidedCollectorLauncherUsesStrictDefaults() {
  ...
  assert.match(script, /cd \/d "%REPO%\\simulator\\python" \|\| exit \/b 1/);
...
function testStatusLauncherUsesGuidedStatusTool() {
  ...
  assert.match(script, /"%NODE%" "%REPO%\\simulator\\tools\\communication\\guided_collect_status\.js" %\*/);
}
