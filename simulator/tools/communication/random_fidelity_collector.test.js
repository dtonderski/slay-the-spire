...
const incompleteEvidence = addCollectionMetadata(
  [{ type: "state", step: 1, message: { boundary_schema: 1 } }],
  bossUnlocks,
  7,
  "SEED7",
  ...
  "test-source-v1",
  7,
  { validateStateSchemas: false },
);
assert.strictEqual(incompleteEvidence[1].message.boundary_schema, 1);
...
const summary = {
  ...
  potions: [
    { index: 0, can_use: true, can_discard: true, requires_target: true },
    { index: 1, id: "LiquidMemories", can_use: true, can_discard: true, requires_target: false },
    { index: 2, id: "PowerPotion", can_use: true, can_discard: true, requires_target: false },
    { index: 3, id: "GamblersBrew", can_use: true, can_discard: true, requires_target: false },
  ],
};
...
assert.deepStrictEqual(
  enumerateGameplayActions({ available_commands: ["cancel"] }),
  ["CANCEL"],
);
assert.deepStrictEqual(
  enumerateGameplayActions({
...
const eligibleActions = enumerateGameplayActions(summary);
assert.deepStrictEqual(
  eligibleActions.map((_action, index) =>
    sampleRandomAction(summary, () => (index + 0.5) / eligibleActions.length).command),
  eligibleActions,
);
for (const [index, command] of eligibleActions.entries()) {
  const sample = sampleRandomAction(summary, () => (index + 0.5) / eligibleActions.length);
  assert.strictEqual(sample.command, command);
  assert.strictEqual(sample.selectedIndex, index);
  assert.deepStrictEqual(sample.actions, eligibleActions);
}
...
async function testIncompleteRunIsSealedAsEvidence() {
  const directory = fs.mkdtempSync(path.join(require("os").tmpdir(), "random-fidelity-hang-"));
  try {
    const bridgeTrace = path.join(directory, "bridge.jsonl");
    fs.writeFileSync(
      bridgeTrace,
      [
        { type: "action", step: 41, command: "START_VERIFY IRONCLAD 0 HANG 10000" },
        { type: "action", step: 42, command: "PLAY 5", command_meta: { command_id: "hang" } },
      ].map((record) => JSON.stringify(record)).join("\n") + "\n",
    );
    const capturedTrace = path.join(directory, "traces", "HANG-p17.jsonl");
    await materializeRunEvidence({
      bridgeTracePath: bridgeTrace,
      runTraceStartOffset: 0,
      destination: capturedTrace,
      bossUnlocks,
      policySeed: 17,
      gameSeed: "HANG",
      startingHp: 10000,
      profile: { note_upgrades: 0, final_act_available: true },
      sourceVersion: "test-source-v1",
      boundarySchema: 7,
      settled: false,
    });
    const persisted = fs.readFileSync(capturedTrace, "utf8");
    assert.match(persisted, /"command":"PLAY 5"/);
    assert.strictEqual(fs.readdirSync(path.dirname(capturedTrace)).some((name) => name.includes(".tmp-")), false);
  } finally {
    fs.rmSync(directory, { recursive: true, force: true });
  }
}

async function testWindowsBridgePathFallsBackToCanonicalSession() {
  if (process.platform === "win32") return;
  const directory = fs.mkdtempSync(path.join(require("os").tmpdir(), "bridge-session-"));
  try {
    const advertised = "Z:\\legacy-layout\\session\\raw_bridge_current.jsonl";
    const canonical = path.join(directory, "raw_bridge_current.jsonl");
    fs.writeFileSync(canonical, "");
    assert.strictEqual(localBridgeTracePath(advertised, directory), canonical);
  } finally {
    fs.rmSync(directory, { recursive: true, force: true });
  }
}
...
Promise.all([
  testOffsetRunRead(),
  testIncompleteRunIsSealedAsEvidence(),
  testWindowsBridgePathFallsBackToCanonicalSession(),
])
