...
| --- | --- | --- | --- |
| Orb Walker | Laser `10` / `11`, Claw `15` / `16` | Fixed: executable Laser/Claw now use public A0/A2 values, and deterministic Claw is a plain attack without Burn insertion | The original swapped-damage/Burn mismatch is resolved; HP range semantics remain listed separately below. |
...
| Gremlin Nob | Rush `14` / `16`; Skull Bash `6` / `8`; Bellow buff | Fixed: executable ascension-aware intent now uses `16` Rush and `8` Skull Bash at A3+ | Original ascension damage mismatch is resolved. |
| Lagavulin | Strong Atk `18` / `20` | Fixed: executable ascension-aware intent now uses `20` at A3+ | Original ascension damage mismatch is resolved. |
| Sentry | Beam `9` / `10` | Fixed: executable ascension-aware intent now wires the existing A3 damage constant | Original ascension damage mismatch is resolved. |
| The Guardian | Fierce Bash `32` / `36`; Roll Attack `9` / `10`; Whirlwind; Vent Steam; defensive mode | Fixed: executable ascension-aware intent now uses A4 Fierce Bash and Roll Attack damage; defensive cycle remains simplified | Ascension damage mismatch is resolved; full boss state/move parity remains incomplete. |
| Bronze Automaton | Flail `7` / `8` x2; Hyper Beam `45` / `50`; Boost block 9; Spawn Orbs | Fixed: executable ascension-aware intent now uses A4 Flail and Hyper Beam damage; spawn action is still represented with `SummonGremlins`; fixed orb HPs remain | Ascension damage mismatch is resolved; action identity/orb spawn completeness remains open. |
| Bronze Orb | Beam 8; Support Beam block 12; Stasis | Fixed: deterministic and target-helper Support Beam block now grant 12 block | Original Support Beam block mismatch is resolved. |
| Mugger | Smoke Bomb public block is 28 | Fixed: executable Smoke Bomb setup block now uses 28 at A0 and high ascension | Original Smoke Bomb block mismatch is resolved. |
...
| GREMLINNOB | Gremlin Nob | Exordium elite | partial | Public HP `82-86` / `85-90`; local fixture HP `82`. Public Rush `14/16`, Skull Bash `6/8`, Bellow. Fixed after audit: executable A3+ Skull Bash/Rush damage is wired; Enrage/Anger on player Skills is modeled at 2. |
...
| LAGAVULIN | Lagavulin | Exordium elite | partial | Public attack `18/20`, Siphon Soul, Open, Idle. Fixed after audit: executable A3+ Strong Attack damage is wired; local sleep/wake/siphon exists. |
...
| SENTRY | Sentry | Exordium elite | partial | Public Bolt and Beam `9/10`. Fixed after audit: executable A3+ attack damage is wired. Local artifact and Beam/Dazed alternation exist. |
