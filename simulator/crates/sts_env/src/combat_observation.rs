...
use sts_core::adapter_internals::{
    card::CardInstance,
    combat::{
        turn_powers::monster_damage_to_player, CombatDecisionState, CombatOrb, CombatPhase,
        CombatState, DiscardSelectPurpose, DrawSelectPurpose, ExhaustSelectPurpose,
        HandSelectPurpose, MonsterIntent, MonsterState, PotionCardRewardKind, SlimeSize,
    },
    content::{cards::get_card_definition, monsters::get_monster_definition},
    effective_card_cost_with_corruption,
    potion::Potion,
    power::PlayerPowers,
...
fn unmodeled_or_unknown_public_content(
    content_id: sts_core::adapter_internals::ContentId,
) -> FairObservationError {
    match sts_core::adapter_internals::run::reward::any_color_reward_card_key(content_id) {
    ...
    let selection = match decision {
        ...
        CombatDecisionState::HandSelect { state, .. } => selection_from_source_indices(
            ...
            mapped_source_indices(combat.piles.hand.len(), |ui_index| {
                sts_core::adapter_internals::combat::transition::hand_select_ui_to_hand_index(
                    combat, ui_index,
                )
            })?,
            canonical_slot_set(
        ...
        CombatDecisionState::DrawSelect { state } => selection_from_source_indices(
            ...
            mapped_source_indices(combat.piles.draw_pile.len(), |ui_index| {
                sts_core::adapter_internals::combat::transition::draw_select_ui_to_draw_index(
                    combat, ui_index,
                )
            })?,
            canonical_slot_set(
        ...
        CombatDecisionState::DiscardSelect { state } => {
            selection_from_source_indices(
                ...
                mapped_source_indices(combat.piles.discard_pile.len(), |ui_index| {
                    sts_core::adapter_internals::combat::transition::discard_select_ui_to_discard_index(combat, ui_index)
                })?,
                canonical_slot_set(
                    state
        ...
        CombatDecisionState::ExhaustSelect { state } => {
            ...
            selection_from_source_indices(
                ...
                mapped_source_indices(cards.len(), |ui_index| {
                    sts_core::adapter_internals::combat::transition::exhaust_select_ui_to_hand_index(
                        &mapping_combat,
                        ui_index,
                    )
                })?,
...
fn mapped_source_indices(
    maximum_count: usize,
    mut map: impl FnMut(usize) -> sts_core::adapter_internals::SimResult<usize>,
    ...
    for ui_index in 0..maximum_count {
        match map(ui_index) {
            ...
            Err(sts_core::adapter_internals::SimError::IllegalAction(message))
                if message.ends_with("index out of range") =>
            {
                break
            }
            Err(sts_core::adapter_internals::SimError::UnknownContent(content_id)) => {
...
mod tests {
    ...
    fn canonical_selection_does_not_reveal_source_pile_order() {
        ...
        combat.decision = Some(CombatDecisionState::DrawSelect {
            state: sts_core::adapter_internals::combat::DrawSelectState {
    ...
    fn invalid_selection_mapping_returns_a_coarse_error() {
        ...
        run.combat.as_mut().expect("combat").decision = Some(CombatDecisionState::ExhaustSelect {
            state: sts_core::adapter_internals::combat::ExhaustSelectState {
    ...
    fn hand_selection_reuses_core_candidate_filtering_without_exposing_source_id() {
        ...
        combat.decision = Some(CombatDecisionState::HandSelect {
            state: sts_core::adapter_internals::combat::HandSelectState {
    ...
    fn serialized_schema_matches_the_path_sensitive_allowlist() {
        ...
        rich.monsters[0].slime_size = Some(sts_core::adapter_internals::SlimeSize::Small);
        ...
        for (label, observation) in [
            ...
            crate::fair_json_allowlist::check_schema(
                &value,
        ...
        let error = crate::fair_json_allowlist::check_schema(
            &leaked,
            &crate::fair_json_allowlist::FAIR_COMBAT_OBSERVATION_SCHEMA,
            "$",
        )
