use sts_core::adapter_internals::run::event::{
    neow_screen_for_stage, Event, MatchAndKeepCard, MatchAndKeepState,
};
...
#[test]
fn player_authority_is_exclusive_and_moves_across_combat() {
    let mut malformed = RunState::combat_fixture();
    malformed.run_player = Some(
        malformed
            .combat
            .as_ref()
            .expect("combat")
    ...
    let mut ownerless = RunState::map_fixture();
    ownerless.run_player = None;
    assert_eq!(
        ownerless.validate(),
        Err(SimError::InvalidState("run has no player owner"))
    );

    let mut run = RunState::map_fixture();
    run.hp = 41;
    run.max_hp = 83;
    run.relics = vec![Relic::BurningBlood, Relic::MarkOfBloom];
    let mut combat = apply_map_action_on_run(
        &run,
        MapAction::ChooseNode {
            node_id: MapNodeId::new(1),
        },
    )
    .expect("combat entry moves the run owner");
    assert!(combat.run_player.is_none());
    assert_eq!(combat.hp, 41);
    assert_eq!(combat.max_hp, 83);
    assert!(combat.relics.contains(&Relic::MarkOfBloom));

    let active = combat.combat.as_mut().expect("active combat");
    active.player.hp = 37;
    active.player.max_hp = 87;
    active.phase = CombatPhase::Won;
    for monster in &mut active.monsters {
        monster.hp = 0;
        monster.alive = false;
    }
    enter_reward_screen(&mut combat).expect("reward exit returns the combat owner");
    assert!(combat.combat.is_none());
    assert!(combat.run_player.is_some());
    assert_eq!(combat.hp, 37);
    assert_eq!(combat.max_hp, 87);
    assert!(combat.relics.contains(&Relic::MarkOfBloom));
}
...
fn orphaned_run_screens_and_room_state_fail_validation() {
    ...
    event.event = Some(sts_core::adapter_internals::event_screen(
        Event::GoldenShrine,
    ));
    assert_eq!(
        event.validate(),
    ...
    sts_core::adapter_internals::enter_shop_room(&mut shop)
        .expect("shop fixture generation succeeds");
...
fn match_and_keep_authoritative_state_is_validated() {
    ...
    run.event = Some(sts_core::adapter_internals::event_screen(
        Event::MatchAndKeep,
    ));
...
fn known_prismatic_reward_identity_is_valid_in_reward_and_deck() {
    ...
    combat.player.authority = run.run_player.take().expect("run player owner");
