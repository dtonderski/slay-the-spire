...
fn hand_card_id(state: &CombatState, content_id: sts_core::adapter_internals::ContentId) -> CardId {
    state
        .piles
        .hand
        .iter()
...
fn the_bomb_has_base_colorless_rare_skill_values() {
    ...
    assert_eq!(
        ...
        Some((
            CardType::Skill,
            sts_core::adapter_internals::card::CardRarity::Rare
        ))
    );
}
...
fn the_bomb_explodes_after_three_end_turn_ticks() {
    ...
    state = sts_core::adapter_internals::end_player_turn(&state).expect("supported monster intent");
...
fn the_bomb_breaking_block_applies_hand_drill_vulnerable() {
    ...
    state
        .relics
        .push(sts_core::adapter_internals::relic::Relic::HandDrill);
    state.bomb_timers = vec![sts_core::adapter_internals::combat::BombTimer {
        turns_remaining: 1,
    ...
    for monster in &mut state.monsters {
        ...
        monster.intent = sts_core::adapter_internals::MonsterIntent::Stun;
    }

    let next =
        sts_core::adapter_internals::end_player_turn(&state).expect("bomb explodes through block");
    ...
    for monster in &next.monsters {
        ...
        assert_eq!(
            ...
            1 + sts_core::adapter_internals::relic::HAND_DRILL_VULNERABLE - 1,
