...
fn normality_in_hand_blocks_fourth_card_play() {
    ...
    assert!(matches!(
        error,
        sts_core::adapter_internals::SimError::IllegalAction(_)
    ));
}
...
fn pen_nib_doubles_only_the_original_double_tapped_bash() {
    ...
    assert_eq!(next.pen_nib_attacks_played, 1);
}
...
fn trip_plus_targets_all_enemies_without_selection() {
    ...
    assert_eq!(
        err,
        sts_core::adapter_internals::SimError::IllegalAction(
...
fn discovery_plus_spends_one_energy_and_delays_non_exhausting_source() {
    ...
    assert_eq!(
        match next.decision.as_ref() {
            Some(sts_core::adapter_internals::CombatDecisionState::DiscoveryCardReward {
                source_card,
                ..
    ...
    sts_core::adapter_internals::combat::transition::close_discovery_card_reward_source(&mut next)
