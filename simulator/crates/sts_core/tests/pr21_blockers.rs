...
fn conclude_hits_all_enemies_before_forcing_end_turn() {
    ...
    state.piles.draw_pile = (101..=105)
        .map(|id| {
            CardInstance::new(
                CardId::new(id),
                sts_core::adapter_internals::content::cards::STRIKE_R_ID,
            )
        })
        .collect();
...
fn dark_evoke_uses_complete_unmodified_death_aftermath() {
    ...
    bronze_orb.stasis_card = Some(CardInstance::new(
        CardId::new(200),
        sts_core::adapter_internals::content::cards::BASH_ID,
    ));
    state.monsters[0].hp = 20;
    state.monsters[0].max_hp = 20;
    ...
    assert!(next
        ...
        .hand
        .iter()
        .any(|card| card.content_id == sts_core::adapter_internals::content::cards::BASH_ID));
}
