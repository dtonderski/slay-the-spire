...
fn anchor_block_stacks_with_defend() {
    ...
    let defend_id = combat
        ...
        .hand
        .iter()
        .find(|card| card.content_id == sts_core::adapter_internals::content::cards::DEFEND_R_ID)
        .expect("defend in hand")
        .id;

    let next = sts_core::adapter_internals::apply_combat_action(
        &combat,
        CombatAction::PlayCard {
            card_id: defend_id,
            target: None,
...
fn ink_bottle_counter_tracks_card_plays_in_combat() {
    ...
    assert_eq!(next.ink_bottle_cards_played, 1);
}
...
fn ink_bottle_draws_card_on_tenth_play() {
    ...
    combat.ink_bottle_cards_played = INK_BOTTLE_THRESHOLD - 1;
    for index in 0..6 {
        combat
            .piles
            .hand
...
fn ink_bottle_counters_round_trip_through_combat_json() {
    let run = RunState::combat_fixture_with_relics(vec![Relic::InkBottle]);
    let mut combat = run.combat.expect("combat initialized");
    combat.ink_bottle_cards_played = 4;

    let json = serde_json::to_string(&combat).expect("combat serializes");
    let restored: sts_core::adapter_internals::CombatState =
        serde_json::from_str(&json).expect("combat deserializes");

    assert_eq!(restored.relics, vec![Relic::InkBottle]);
    assert_eq!(restored.ink_bottle_cards_played, 4);
}
...
fn lizard_tail_availability_round_trips_through_combat_json() {
    ...
    let mut combat = run.combat.expect("combat initialized");
    combat.lizard_tail_used = false;

    let json = serde_json::to_string(&combat).expect("combat serializes");
    let restored: sts_core::adapter_internals::CombatState =
        serde_json::from_str(&json).expect("combat deserializes");

    assert!(!restored.lizard_tail_used);
}
...
fn strawberry_pickup_increases_current_and_max_hp() {
    ...
    assert_eq!(run.max_hp, 80 + STRAWBERRY_MAX_HP);
    assert_eq!(run.hp, 40 + STRAWBERRY_MAX_HP);
}
...
fn strawberry_round_trips_through_run_state_json() {
    ...
    assert_eq!(restored.max_hp, 80 + STRAWBERRY_MAX_HP);
}
...
fn coffee_dripper_disables_rest_heal() {
    ...
    assert_eq!(
        err,
        sts_core::adapter_internals::SimError::IllegalAction("heal is not available")
    );
}
