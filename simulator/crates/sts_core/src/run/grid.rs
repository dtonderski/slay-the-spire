...
fn transform_neow_cards(run: &mut RunState, cards: &[CardInstance]) -> SimResult<()> {
    ...
    // commit only when the stage-2 Leave screen settles that effect. Keep the
    // source list, RNG endpoint, and construction-time Omamori state typed so
    // validation can recompute the pending result without consulting a trace.
    let mut next = run.clone();
    for card in cards {
        next.remove_deck_card(card.id)
            .expect("transform selected a deck card");
    }
    next.pending_neow_transform = Some(super::state::PendingNeowTransform {
        sources: cards.to_vec(),
        neow_rng_counter: reward.neow_rng_counter,
        omamori_charges_used: next.omamori_charges_used,
    });
    for content_id in transformed {
...
mod tests {
    ...
    fn neow_single_transform_requires_confirm_and_defers_obtain_until_leave() {
        ...
        assert_eq!(
            before_leave
                .pending_neow_transform
                .as_ref()
                .map(|transform| transform.sources.as_slice()),
        ...
        let mut tampered = before_leave.clone();
        tampered
            .pending_neow_transform
            .as_mut()
            .expect("typed Neow provenance")
            .neow_rng_counter += 1;
        assert!(tampered.validate().is_err());

        let snapshot = serde_json::to_string(&before_leave).expect("serialize pending Neow state");
        let restored: RunState =
            serde_json::from_str(&snapshot).expect("restore pending Neow state");
        assert_eq!(restored, before_leave);
        ...
        assert!(left.pending_obtain_cards.is_empty());
        assert!(left.pending_neow_transform.is_none());
        assert_eq!(left.deck.len(), original_deck.len());
        assert_eq!(
