...
pub fn open_pandoras_box_grid(run: &mut RunState) -> SimResult<()> {
    run.transactional(open_pandoras_box_grid_inner)
}
...
pub fn select_grid_card(run: &RunState, index: usize) -> SimResult<RunState> {
    ...
    apply_validated_grid_select(run.clone(), index)
}

pub(crate) fn apply_validated_grid_select(mut next: RunState, index: usize) -> SimResult<RunState> {
    let purpose = next
        .card_grid
        .as_ref()
        .expect("validated card grid")
        .purpose;

    if let Some(required) = grid_multi_select_count(purpose) {
...
pub fn cancel_grid(run: &RunState) -> SimResult<RunState> {
    ...
    Ok(apply_validated_grid_cancel(run.clone()))
}
...
pub fn confirm_grid(run: &RunState) -> SimResult<RunState> {
    ...
    apply_validated_grid_confirmation(run.clone())
}

pub(crate) fn apply_validated_grid_confirmation(mut next: RunState) -> SimResult<RunState> {
    let grid = next
        .card_grid
        .as_ref()
        .expect("validated card grid")
