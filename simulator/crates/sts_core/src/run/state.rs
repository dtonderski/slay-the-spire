use crate::{
    ...
    combat::state::{PlayerAuthorityState, BASE_PLAYER_ENERGY},
...
mod tests {
    ...
    fn run_validation_rejects_invalid_pending_event_combat_rewards() {
        ...
        combat
            .install_combat_owner(CombatState::initial_fixture())
            .expect("combat owner installs");
    ...
    fn gold_gain_and_floor_relic_overflow_roll_back_exactly() {
        ...
        late_failure.max_hp = i32::MAX;
        late_failure.hp = i32::MIN;
    ...
    fn combat_initialization_rejects_starting_relic_arithmetic_overflow() {
        ...
        red_skull_run.hp = red_skull_run.max_hp / 2;
        let mut base = CombatState::initial_fixture();
        base.player.powers.strength = i32::MAX;
        base.validate().expect("input combat is valid");
        assert_eq!(
...
#[cfg(test)]
thread_local! {
    static WHOLE_RUN_CLONE_COUNT: std::cell::Cell<usize> = const { std::cell::Cell::new(0) };
}

#[cfg(test)]
#[derive(Debug, Default, PartialEq, Eq)]
struct RunCloneProbe;

#[cfg(test)]
impl Clone for RunCloneProbe {
    fn clone(&self) -> Self {
        WHOLE_RUN_CLONE_COUNT.with(|count| count.set(count.get() + 1));
        Self
    }
}

#[cfg(test)]
pub(crate) fn reset_whole_run_clone_count() {
    WHOLE_RUN_CLONE_COUNT.with(|count| count.set(0));
}

#[cfg(test)]
pub(crate) fn whole_run_clone_count() -> usize {
...
pub struct RunState {
    ...
    /// Present exactly when combat is not the active player owner.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub run_player: Option<PlayerAuthorityState>,
    pub gold: i32,
...
impl std::ops::Deref for RunState {
    type Target = PlayerAuthorityState;

    fn deref(&self) -> &Self::Target {
        match (&self.run_player, &self.combat) {
            (Some(player), None) => player,
            (None, Some(combat)) => &combat.player.authority,
            _ => panic!("RunState player authority invariant is violated"),
        }
    }
}

impl std::ops::DerefMut for RunState {
    fn deref_mut(&mut self) -> &mut Self::Target {
        match (&mut self.run_player, &mut self.combat) {
            (Some(player), None) => player,
            (None, Some(combat)) => &mut combat.player.authority,
            _ => panic!("RunState player authority invariant is violated"),
        }
    }
}
...
impl RunState {
    pub(crate) fn transactional<T>(
        ...
        if super::decision::outer_run_transaction_active() {
    ...
    fn transactional_mutation(
        &mut self,
        operation: impl FnOnce(&mut Self) -> SimResult<()>,
    ) -> SimResult<()> {
        self.transactional(operation)
    }

    /// Publish a newly initialized combat by relinquishing the run-owned player.
    pub(crate) fn install_combat_owner(&mut self, combat: CombatState) -> SimResult<()> {
        if self.combat.is_some() || self.run_player.is_none() {
            return Err(SimError::InvalidState(
                "combat entry requires exactly one run player owner",
            ));
        }
        self.run_player = None;
        self.combat = Some(combat);
        Ok(())
    }

    /// Consume the active combat and return its player data to the run.
    pub(crate) fn leave_combat(&mut self) -> SimResult<()> {
        if self.run_player.is_some() {
            return Err(SimError::InvalidState(
                "combat exit encountered an existing run player owner",
            ));
        }
        let combat = self.combat.take().ok_or(SimError::InvalidState(
            "combat exit requires an active combat owner",
        ))?;
        self.run_player = Some(combat.player.authority);
        Ok(())
    }

    pub(crate) fn leave_combat_if_active(&mut self) {
        if self.combat.is_some() {
            self.leave_combat()
                .expect("active combat is the sole player owner");
        }
    }
    ...
    pub fn validate(&self) -> SimResult<()> {
        ...
        match (&self.phase, &self.run_player, &self.combat) {
            ...
            (RunPhase::Combat, Some(_), Some(_)) => {
                return Err(SimError::InvalidState(
                    "combat phase has both run and combat player owners",
                ));
            }
            (RunPhase::Combat, Some(_), None) => {
                return Err(SimError::InvalidState("combat phase has no combat owner"));
            }
            (RunPhase::Combat, None, None) | (_, None, None) => {
                return Err(SimError::InvalidState("run has no player owner"));
            }
            (_, None, Some(_)) => {
                return Err(SimError::InvalidState(
                    "combat player owner exists outside combat phase",
                ));
            }
            (_, Some(_), Some(_)) => {
                return Err(SimError::InvalidState("run has two player owners"));
            }
            (_, Some(_), None) => {}
        }
        ...
        if self.max_hp <= 0 || self.hp < 0 || self.hp > self.max_hp {
    ...
    pub fn init_combat(&self, base: CombatState) -> SimResult<CombatState> {
        ...
        combat.player.authority = self
            ...
            .ok_or(SimError::InvalidState(
                "combat initialization requires the run player owner",
            ))?
            .clone();
    ...
    pub fn combat_fixture_with_options(relics: Vec<Relic>, ascension: u8) -> Self {
        ...
        let combat = run
            .init_combat(CombatState::initial_fixture())
            .expect("combat initializes");
        run.install_combat_owner(combat)
            .expect("combat owner installs");
        run
    }
    ...
    pub fn flush_pending_obtain_cards(&mut self) -> SimResult<()> {
        self.transactional_mutation(|next| {
    ...
    pub fn add_deck_card(&mut self, card: CardInstance) -> SimResult<()> {
        self.transactional_mutation(|next| next.add_deck_card_inner(card))
    }
    ...
    pub fn add_finalized_reward_deck_card(&mut self, card: CardInstance) -> SimResult<()> {
        self.transactional_mutation(|next| next.add_deck_card_inner_with_options(card, true, false))
    }
    ...
    fn apply_card_added_relics(&mut self, content_id: ContentId) -> SimResult<()> {
        ...
        if self.relics.contains(&Relic::DarkstonePeriapt) && is_curse_content_id(content_id) {
            // AbstractCreature.increaseMaxHp heals the same amount through
            // heal(amount, true). MagicFlower.onPlayerHeal applies only while
            // the current room is COMBAT (6 * 1.5 → 9).
            self.max_hp = checked_run_add(self.max_hp, DARKSTONE_PERIAPT_MAX_HP)?;
            ...
            self.hp = checked_run_add(self.hp, heal)?.min(self.max_hp);
        }
        Ok(())
    }

    /// Commit combat-queued obtains to the run deck while applying their
    /// player effects directly to the combat owner supplied by the caller.
    pub(crate) fn flush_pending_combat_obtains_into(
        &mut self,
        authority: &mut PlayerAuthorityState,
    ) -> SimResult<()> {
        let pending = std::mem::take(&mut self.pending_combat_obtain_cards);
        for content_id in pending {
            if authority.relics.contains(&Relic::Omamori)
                && is_curse_content_id(content_id)
                && self.omamori_charges_used < OMAMORI_CHARGES
            {
                self.omamori_charges_used = self
            ...
            let id = CardId::new(self.next_card_instance_id()?);
            let mut card = pending_obtain_card_instance(id, content_id);
            validate_run_card_content(&card)?;
            if self.deck.iter().any(|existing| existing.id == card.id) {
                return Err(SimError::InvalidState(
                    "duplicate run deck card instance ID",
            ...
            card = self.card_after_card_add_relics(card)?;
            let added_content_id = card.content_id;
            self.deck.push(card);
            if authority.relics.contains(&Relic::CeramicFish) {
                self.gain_gold(CERAMIC_FISH_GOLD)?;
            }
            if authority.relics.contains(&Relic::DarkstonePeriapt)
                && is_curse_content_id(added_content_id)
            {
                authority.max_hp = checked_run_add(authority.max_hp, DARKSTONE_PERIAPT_MAX_HP)?;
                let heal = if authority.relics.contains(&Relic::MarkOfBloom) {
                    0
                } else {
                    combat_healing_amount_with_relics(DARKSTONE_PERIAPT_MAX_HP, &authority.relics)
                };
                authority.hp = checked_run_add(authority.hp, heal)?.min(authority.max_hp);
            }
        }
        Ok(())
    }
    ...
    pub fn heal_player(&mut self, amount: i32) -> SimResult<()> {
        if amount > 0 && !self.has_mark_of_bloom() {
            ...
            if missing_hp > 0 {
                self.hp = checked_run_add(self.hp, amount.min(missing_hp))?;
            }
        }
        Ok(())
    }
    ...
    pub fn gain_max_hp(&mut self, amount: i32) -> SimResult<()> {
        ...
        let player_max_hp = checked_run_add(self.max_hp, amount)?;
        let player_hp = if self.has_mark_of_bloom() {
            self.hp
        } else {
            checked_run_add(self.hp, amount)?
        };
    ...
    pub fn increase_max_hp_without_healing(&mut self, amount: i32) -> SimResult<()> {
        ...
        self.max_hp = checked_run_add(self.max_hp, amount)?;
        Ok(())
    }
    ...
    pub fn gain_gold(&mut self, amount: i32) -> SimResult<()> {
        self.transactional_mutation(|next| next.gain_gold_inner(amount))
    }
    ...
    pub fn gain_relic(&mut self, relic: Relic) -> SimResult<()> {
        self.transactional_mutation(|next| next.gain_relic_inner(relic))
    }
    ...
    fn gain_relic_inner(&mut self, relic: Relic) -> SimResult<()> {
        ...
        match relic {
            Relic::Strawberry => {
                self.max_hp = checked_run_add(self.max_hp, STRAWBERRY_MAX_HP)?;
                self.heal_player(STRAWBERRY_MAX_HP)?;
            }
            Relic::Pear => {
                self.max_hp = checked_run_add(self.max_hp, PEAR_MAX_HP)?;
                self.heal_player(PEAR_MAX_HP)?;
            }
            Relic::Mango => {
                self.max_hp = checked_run_add(self.max_hp, MANGO_MAX_HP)?;
            ...
            Relic::LeesWaffle => {
                self.max_hp = checked_run_add(self.max_hp, LEES_WAFFLE_MAX_HP)?;
                self.heal_player(self.max_hp)?;
            }
            Relic::CoffeeDripper => {
            ...
            Relic::TinyHouse => {
                self.max_hp = checked_run_add(self.max_hp, TINY_HOUSE_MAX_HP)?;
