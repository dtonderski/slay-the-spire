...
pub fn apply_neow_boss_swap(run: &mut RunState) -> SimResult<NeowBossSwapReward> {
    run.transactional(apply_neow_boss_swap_inner)
}
...
pub fn apply_neow_relic_reward(
    ...
    run.transactional(|next| apply_neow_relic_reward_inner(next, reward))
}
    ...
    run.transactional(|next| {
        match drawback {
            ...
            NeowDrawback::TenPercentHpLoss => {
                let amount = ten_percent(next.max_hp);
                lose_max_hp(next, amount)?;
            }
            NeowDrawback::NoGold => next.gold = 0,
            NeowDrawback::PercentDamage => {
                next.hp = next
                    .hp
                    .checked_sub(percent_damage(next.max_hp))
