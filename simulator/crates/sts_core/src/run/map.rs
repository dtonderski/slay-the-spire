...
pub fn apply_map_action_on_run(run: &RunState, action: MapAction) -> SimResult<RunState> {
    ...
    apply_validated_map_action_on_run(run.clone(), action)
}

pub(crate) fn apply_validated_map_action_on_run(
    mut next: RunState,
    action: MapAction,
) -> SimResult<RunState> {
...
fn apply_event_room_outcome(run: &mut RunState, last_room_was_shop: bool) -> SimResult<()> {
    run.transactional(|next| apply_event_room_outcome_inner(next, last_room_was_shop))
}
...
mod tests {
    ...
    fn hjtgfct_map_path_third_pick_enters_event_room() {
        ...
        run.leave_combat()
            .expect("first combat owner returns to run");
        run.phase = RunPhase::Idle;
        ...
        run.leave_combat()
            .expect("second combat owner returns to run");
        run.phase = RunPhase::Idle;
