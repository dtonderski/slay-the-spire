...
pub fn apply_hand_select_choice(run: &RunState, index: usize) -> SimResult<RunState> {
    ...
    apply_validated_hand_select_choice_owned(run.clone(), index)
}

pub(crate) fn apply_validated_hand_select_choice_owned(
    mut next: RunState,
    index: usize,
) -> SimResult<RunState> {
...
pub fn apply_hand_select_confirm(run: &RunState) -> SimResult<RunState> {
    validate_hand_select_confirm(run)?;
    apply_validated_hand_select_confirm_owned(run.clone())
}

pub(crate) fn apply_validated_hand_select_confirm_owned(mut next: RunState) -> SimResult<RunState> {
    let mut combat = next.combat.as_ref().expect("validated combat").clone();
...
pub fn apply_hand_select_confirm_without_retrieval(run: &RunState) -> SimResult<RunState> {
    validate_hand_select_confirm(run)?;
    apply_validated_hand_select_confirm_without_retrieval_owned(run.clone())
}

pub(crate) fn apply_validated_hand_select_confirm_without_retrieval_owned(
    mut next: RunState,
) -> SimResult<RunState> {
    let mut combat = next.combat.as_ref().expect("validated combat").clone();
    let exhaust_before = combat.piles.exhaust_pile.len();
...
pub fn apply_draw_select_choice(run: &RunState, index: usize) -> SimResult<RunState> {
    ...
    apply_validated_draw_select_choice_owned(run.clone(), index)
}

pub(crate) fn apply_validated_draw_select_choice_owned(
    mut next: RunState,
    index: usize,
) -> SimResult<RunState> {
    let mut combat = next.combat.as_ref().expect("validated combat").clone();
...
pub fn apply_draw_select_confirm(run: &RunState) -> SimResult<RunState> {
    ...
    apply_validated_draw_select_confirm_owned(run.clone())
}

pub(crate) fn apply_validated_draw_select_confirm_owned(mut next: RunState) -> SimResult<RunState> {
    let mut combat = next.combat.as_ref().expect("validated combat").clone();
    let exhaust_before = combat.piles.exhaust_pile.len();
    let handled_dead_branch_count = confirm_draw_select(&mut combat)?;
    let exhaust_count = combat
...
pub fn apply_discard_select_choice(run: &RunState, index: usize) -> SimResult<RunState> {
    ...
    apply_validated_discard_select_choice_owned(run.clone(), index)
}

pub(crate) fn apply_validated_discard_select_choice_owned(
    mut next: RunState,
    index: usize,
) -> SimResult<RunState> {
    let purpose = next
...
pub fn apply_discard_select_confirm(run: &RunState) -> SimResult<RunState> {
    ...
    apply_validated_discard_select_confirm_owned(run.clone())
}

pub(crate) fn apply_validated_discard_select_confirm_owned(
    mut next: RunState,
) -> SimResult<RunState> {
    let mut combat = next.combat.as_ref().expect("validated combat").clone();
    let exhaust_before = combat.piles.exhaust_pile.len();
...
pub fn apply_exhaust_select_choice(run: &RunState, index: usize) -> SimResult<RunState> {
    ...
    apply_validated_exhaust_select_choice_owned(run.clone(), index)
}

pub(crate) fn apply_validated_exhaust_select_choice_owned(
    mut next: RunState,
    index: usize,
) -> SimResult<RunState> {
    let purpose = next
...
pub fn apply_exhaust_select_confirm(run: &RunState) -> SimResult<RunState> {
    ...
    apply_validated_exhaust_select_confirm_owned(run.clone())
}

pub(crate) fn apply_validated_exhaust_select_confirm_owned(
    mut next: RunState,
) -> SimResult<RunState> {
    let mut combat = next.combat.as_ref().expect("validated combat").clone();
    let before = combat.clone();
...
pub fn apply_combat_card_reward_choice(run: &RunState, index: usize) -> SimResult<RunState> {
    validate_combat_card_reward_choice(run, index)?;
    apply_validated_combat_card_reward_choice_owned(run.clone(), index)
}

pub(crate) fn apply_validated_combat_card_reward_choice_owned(
    mut next: RunState,
    index: usize,
) -> SimResult<RunState> {
...
pub fn apply_combat_card_reward_skip(run: &RunState) -> SimResult<RunState> {
    ...
    apply_validated_combat_card_reward_skip_owned(run.clone())
}

pub(crate) fn apply_validated_combat_card_reward_skip_owned(
    mut next: RunState,
) -> SimResult<RunState> {
    let combat = next.combat.as_mut().expect("validated combat");
...
pub(crate) fn apply_validated_potion_action_owned(
    mut next: RunState,
    action: RunAction,
) -> SimResult<RunState> {
    match action {
        RunAction::UsePotion { slot, target } => {
            ...
            match potion {
                ...
                Potion::Blood => {
                    if let Some(combat) = next.combat.as_mut() {
                        ...
                        let heal = blood_potion_heal(next.max_hp, multiplier)?;
            ...
            if defer_potion_use_relics {
                // The deferred combat selection applies potion-use relics when it closes.
            } else if let Some(combat) = next.combat.as_mut() {
                crate::relic::apply_potion_use_relics_to_combat(combat)?;
            } else {
                let max_hp = next.max_hp;
                let relics = next.relics.clone();
                crate::relic::apply_potion_use_relics_to_run_hp(&mut next.hp, max_hp, &relics);
            }
            let won = next
                .combat
                .as_ref()
...
mod tests {
    ...
    fn nilry_skip_persists_lizard_tail_consumed_during_resumed_monster_turn() {
        ...
        assert!(combat.lizard_tail_used);
    }
    ...
    fn discovery_retrieve_early_magnetism_source_remaining_five_with_leftover_magnetism_burns_fifteen_generations(
        ...
        let next = apply_combat_card_reward_choice(&run, 0)
            .expect("remaining-5 Magnetism Discovery with leftover Magnetism card");
        let combat = next.combat.expect("combat remains open");
        assert_eq!(
            combat.rng.card_random_rng.counter(),
            62,
    ...
    fn toolbox_choice_activates_queued_gambling_chip_selection() {
        ...
        run.install_combat_owner(combat)
            .expect("Toolbox combat owner installs");
