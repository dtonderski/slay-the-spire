...
pub fn apply_combat_card_reward_choice(run: &RunState, index: usize) -> SimResult<RunState> {
    ...
    match decision {
        ...
        CombatDecisionState::DiscoveryCardReward {
            ...
            // the same run). An early-turn deck Discovery while a Magnetism
            // card remains also leaves two pulses (FIDL01416: Magnetism applied
            // before draw, so Enlightenment is first and Discovery is from the
            // deck; next Magnetism is Jack of All Trades, not Panic Button).
            // Remaining hand of 5 with no other generated card stays one pulse
            // (FIDL01582 The Bomb, not Blind). A lone early-turn retrieve
...
fn discovery_post_select_generations(
    ...
    let early_magnetism_turn = combat.player.powers.magnetism > 0
        && combat.relic_counters.cards_played_this_turn <= 2
        && !source_card_play_top;
    let early_magnetism_generated_source =
        early_magnetism_turn && source_card.is_some_and(|card| card.magnetism_generated);
    ...
    if another_discovery_in_hand
        ...
        || (early_magnetism_turn && another_magnetism_card_in_hand)
...
mod tests {
    ...
    #[test]
    fn discovery_retrieve_early_deck_source_with_leftover_magnetism_card_burns_two_generations() {
        ...
        let next = apply_combat_card_reward_choice(&run, 0)
            .expect("early-turn deck Discovery with leftover Magnetism card");
        let combat = next.combat.expect("combat remains open");
        assert_eq!(
            combat.rng.card_random_rng.counter(),
            22,
            "early-turn deck Discovery while a Magnetism card remains burns two discarded generations"
        );
    }
