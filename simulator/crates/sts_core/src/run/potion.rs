...
pub fn apply_combat_card_reward_choice(run: &RunState, index: usize) -> SimResult<RunState> {
    ...
    match decision {
        ...
        CombatDecisionState::DiscoveryCardReward {
            ...
            // of the turn leaves two pulses when another Magnetism-generated
            // card is still in hand (FIDL01787 Transmutation vs Enlightenment
            // after step 325). A lone early-turn Magnetism Discovery stays one
            // pulse (FIDL01787 Flash of Steel vs Good Instincts at step 1115).
...
mod tests {
    ...
    fn discovery_retrieve_with_magnetism_generated_source_burns_two_generations() {
        ...
        combat.piles.hand = (1..=4)
            .map(|id| {
                let mut card = CardInstance::new(CardId::new(id), STRIKE_R_ID);
                if id == 1 {
                    card.magnetism_generated = true;
        ...
        assert_eq!(
            combat.rng.card_random_rng.counter(),
            22,
            "early-turn Magnetism Discovery with another generated card in hand burns two discarded generations"
        );
    }

    #[test]
    fn discovery_retrieve_lone_early_magnetism_generated_source_burns_one_generation() {
        use crate::content::cards::{DISCOVERY_ID, STRIKE_R_ID};
        ...
        assert_eq!(
            combat.rng.card_random_rng.counter(),
            19,
            "lone early-turn Magnetism-generated Discovery burns one discarded generation"
        );
    }
