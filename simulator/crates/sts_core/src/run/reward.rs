use crate::{
    ...
    combat::{
        finish_monster_turn_after_player_revival, start_player_turn,
        transition::apply_validated_combat_action_owned, CombatPhase,
    },
    content::cards::{
...
pub fn enter_relic_reward_screen(run: &mut RunState, kind: CombatRewardKind) -> SimResult<()> {
    run.transactional(|next| enter_relic_reward_screen_inner(next, kind))
}
...
fn enter_relic_reward_screen_inner(run: &mut RunState, kind: CombatRewardKind) -> SimResult<()> {
    ...
    if run.can_gain_potions() {
        ...
        let has_white_beast_statue = run.relics.contains(&Relic::WhiteBeastStatue);
...
pub fn enter_normal_combat_reward_screen(run: &mut RunState) -> SimResult<()> {
    run.transactional(enter_normal_combat_reward_screen_inner)
}
...
fn enter_normal_combat_reward_screen_inner(run: &mut RunState) -> SimResult<()> {
    ...
    let potion_capacity = run.potion_capacity();
    let has_white_beast_statue = run.relics.contains(&Relic::WhiteBeastStatue);
    let potion_offer = if all_monsters_escaped && !has_white_beast_statue {
...
pub fn enter_elite_combat_reward_screen(run: &mut RunState) -> SimResult<()> {
    run.transactional(enter_elite_combat_reward_screen_inner)
}
...
pub fn enter_boss_combat_reward_screen(run: &mut RunState) -> SimResult<()> {
    run.transactional(enter_boss_combat_reward_screen_inner)
}
...
pub fn enter_chest_relic_reward_screen(run: &mut RunState) -> SimResult<()> {
    run.transactional(enter_chest_relic_reward_screen_inner)
}
...
pub fn apply_combat_action_on_run(run: &RunState, action: CombatAction) -> SimResult<RunState> {
    crate::run::decision::validate_run_decision_action(
        run,
        crate::run::decision::RunDecisionAction::Combat(action),
    )?;
    apply_validated_combat_action_on_owned_run(run.clone(), action)
}

pub(crate) fn apply_validated_combat_action_on_owned_run(
    mut next: RunState,
    action: CombatAction,
) -> SimResult<RunState> {
    // RunState's sole player authority is embedded in the active combat. Keep
    // that owner installed while run-level potion/relic settlement reads it;
    // only the combat working value is cloned here, never the whole run.
    let mut combat_for_action = next
        .combat
        .as_ref()
        .ok_or(SimError::InvalidState("combat state is missing"))?
        .clone();
    combat_for_action.relic_counters.fairy_consumed = false;
    combat_for_action.relic_counters.fairy_heal_percent = if next
        .occupied_potion_slots()
        .iter()
        .any(|(_, potion)| *potion == Potion::Fairy)
        && !next.has_mark_of_bloom()
    {
        FAIRY_HEAL_PERCENT
            * if next.relics.contains(&Relic::SacredBark) {
                2
            } else {
                1
            }
    ...
    // Settlement compares combat-local deltas against the pre-transition
    // combat. This clone is deliberately scoped below RunState ownership.
    let before = combat_for_action.clone();
    let next_combat = apply_validated_combat_action_owned(combat_for_action, action, false)?.state;
    let next_combat = settle_run_after_combat_transition(
        &mut next,
        &before,
        next_combat,
        matches!(action, CombatAction::EndTurn),
    )?;
    let combat_won = next_combat.phase == CombatPhase::Won;
    ...
    let colosseum_second_fight = next_combat.monsters.len() == 2
        && next_combat.monsters[0].content_id == TASKMASTER_ID
        && next_combat.monsters[1].content_id == GREMLIN_NOB_ID;
    let event_combat_rng = next_combat.rng.clone();
    ...
    if combat_won {
        if colosseum_first_fight {
            ...
            next.pending_event_combat_rng = Some(event_combat_rng);
        ...
        } else if colosseum_second_fight {
...
pub(crate) fn settle_run_after_combat_transition(
    ...
    apply_looter_theft_to_run_gold(run, before, &mut after);
    apply_combat_gold_gain_to_run(run, before, &mut after)?;
    settle_combat_obtain_actions_after_transition(run, before, &mut after)?;
    ...
    let revived = apply_fairy_if_lethal(run, &mut after)?;
    if revived
        && finish_revived_end_turn
        && after.phase == CombatPhase::WaitingForPlayer
        && after.monsters.iter().any(|monster| monster.alive)
    {
        finish_monster_turn_after_player_revival(&mut after)?;
        start_player_turn(&mut after)?;
    }
...
fn settle_pending_combat_obtain_cards(
    ...
    // during this transition, before its command-ready boundary. The deck is
    // run-owned, while player effects target the sole active combat owner.
    run.flush_pending_combat_obtains_into(&mut after.player.authority)?;
    Ok(())
}
...
fn apply_combat_loss_proceed(mut next: RunState) -> SimResult<RunState> {
...
pub(crate) fn apply_validated_run_action_owned(
    ...
    let next = match action {
        ...
        RunAction::Proceed if next.phase == RunPhase::Shop => {
            super::shop::apply_validated_shop_action_owned(next, action)
        }
        RunAction::Proceed if next.phase == RunPhase::Combat => apply_combat_loss_proceed(next),
        RunAction::Proceed if next.phase == RunPhase::Victory => {
            apply_final_boss_victory_proceed(next)
        }
        RunAction::Proceed => apply_validated_treasure_action_owned(next, action),
        ...
        RunAction::ChooseCombatCardReward { index } => {
            super::potion::apply_validated_combat_card_reward_choice_owned(next, index)
        }
        RunAction::SkipCombatCardReward => {
            super::potion::apply_validated_combat_card_reward_skip_owned(next)
        }
        RunAction::ChooseHandSelect { index } => {
            super::potion::apply_validated_hand_select_choice_owned(next, index)
        }
        RunAction::ConfirmHandSelect => {
            super::potion::apply_validated_hand_select_confirm_owned(next)
        }
        RunAction::ConfirmHandSelectWithoutRetrieval => {
            super::potion::apply_validated_hand_select_confirm_without_retrieval_owned(next)
        }
        RunAction::ChooseDrawSelect { index } => {
            super::potion::apply_validated_draw_select_choice_owned(next, index)
        }
        RunAction::ConfirmDrawSelect => {
            super::potion::apply_validated_draw_select_confirm_owned(next)
        }
        RunAction::ChooseDiscardSelect { index } => {
            super::potion::apply_validated_discard_select_choice_owned(next, index)
        }
        RunAction::ConfirmDiscardSelect => {
            super::potion::apply_validated_discard_select_confirm_owned(next)
        }
        RunAction::ChooseExhaustSelect { index } => {
            super::potion::apply_validated_exhaust_select_choice_owned(next, index)
        }
...
pub fn apply_treasure_action(run: &RunState, action: RunAction) -> SimResult<RunState> {
    ...
    apply_validated_treasure_action_owned(run.clone(), action)
}

fn apply_validated_treasure_action_owned(
    mut next: RunState,
    action: RunAction,
) -> SimResult<RunState> {
    match action {
...
fn apply_final_boss_victory_proceed(mut next: RunState) -> SimResult<RunState> {
...
fn apply_reward_action(run: &RunState, action: RunAction) -> SimResult<RunState> {
    ...
    apply_validated_reward_action_owned(run.clone(), action)
}

fn apply_validated_reward_action_owned(
    mut next: RunState,
    action: RunAction,
) -> SimResult<RunState> {
    match action {
...
mod tests {
    ...
    fn prepare_won_combat_reward_fixture(run: &mut RunState) {
        let mut combat = if let Some(combat) = run.combat.take() {
            combat
        } else {
            let mut combat = CombatState::initial_fixture();
            combat.player.authority = run
                .run_player
                .take()
                .expect("fixture starts with a run player owner");
            combat
        };
        if let Some(authority) = run.run_player.take() {
            combat.player.authority = authority;
        }
        ...
        run.card_grid = None;
        if let Some(authority) = run.run_player.take() {
            combat.player.authority.relics = authority.relics;
        }
    ...
    fn pen_nib_counter_persists_between_run_and_combat() {
        ...
        combat.pen_nib_attacks_played = run.pen_nib_attacks_played;
        if let Some(authority) = run.run_player.take() {
            combat.player.authority.relics = authority.relics;
        }
        run.combat = Some(combat);
    ...
    fn ink_bottle_counter_persists_between_run_and_combat() {
        ...
        combat.relics = run.relics.clone();
        combat.ink_bottle_cards_played = run.ink_bottle_cards_played;
        if let Some(authority) = run.run_player.take() {
            combat.player.authority.relics = authority.relics;
        }
        run.combat = Some(combat);
        ...
        assert_eq!(combat.ink_bottle_cards_played, 0);
        assert_eq!(
    ...
    fn happy_flower_counter_persists_between_run_and_combat_turns() {
        ...
        run.happy_flower_turns = combat.happy_flower_turns;
        if let Some(authority) = run.run_player.take() {
            combat.player.authority.relics = authority.relics;
        }
        run.combat = Some(combat);
        ...
        assert_eq!(combat.happy_flower_turns, 0);
    ...
    fn sundial_counter_persists_between_combats_and_grants_third_shuffle_energy() {
        ...
        let mut combat = run.init_combat(base).expect("combat initializes");
        assert_eq!(combat.sundial_shuffles, 2);
        if let Some(authority) = run.run_player.take() {
            combat.player.authority.relics = authority.relics;
        }
        run.combat = Some(combat);
        ...
        assert_eq!(combat.sundial_shuffles, 3);
    ...
    fn tiny_house_boss_relic_opens_its_room_reward_overlay() {
        ...
        assert_eq!(picked.max_hp, 105);
        assert_eq!(picked.hp, 55);
    ...
    fn singing_bowl_with_mark_of_bloom_raises_max_hp_without_healing() {
        ...
        assert_eq!(run.max_hp, 82);
        assert_eq!(run.hp, 50, "Mark of the Bloom must block Singing Bowl heal");
    }
