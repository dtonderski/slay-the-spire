...
pub fn apply_rest_action(run: &RunState, action: RestAction) -> SimResult<RunState> {
    ...
    apply_validated_rest_action(run.clone(), action)
}

pub(crate) fn apply_validated_rest_action(
    mut next: RunState,
    action: RestAction,
) -> SimResult<RunState> {
    match action {
        RestAction::Heal => {
            let mut heal = rest_heal_amount(next.max_hp);
...
mod tests {
    ...
    use crate::{
        ...
        CardId, CardInstance, RoomKind, RunAction,
    };
