...
pub(crate) use action::apply_validated_event_action;
...
pub(super) fn validate_event_screen_authority(
    ...
    match screen.event {
            ...
            let valid_data = match screen.stage {
                ...
                1 => {
                    (screen.event_data == 0 || screen.event_data == BIG_FISH_MAX_HP_GAIN as u32)
                        || screen.event_data == (run.max_hp / 3) as u32
                }
        ...
        Event::GoldenIdol => {
            ...
            if screen.choices != golden_idol_choices(screen.stage, run.max_hp, run.ascension) {
                return Err(SimError::InvalidState(
    ...
    let valid = match run.event.as_ref() {
        Some(screen) if run.phase == RunPhase::Event => match (screen.event, screen.stage) {
            (Event::GoldenIdol, 2) if screen.event_data == 0 => {
                let zero_hp_result_is_reachable = golden_idol_hp_loss(run.max_hp, run.ascension)
                    == 0
                    || golden_idol_max_hp_loss(run.max_hp, run.ascension) == 0;
...
fn settle_out_of_combat_lethal_hp(run: &mut RunState) -> SimResult<()> {
    if run.hp > 0 || run.phase == RunPhase::Complete || run.combat.is_some() {
        return Ok(());
    }
    ...
    if !run.has_mark_of_bloom() {
            ...
            .find(|(_, potion)| *potion == Potion::Fairy)
        {
            ...
            run.hp =
                revival_hp_with_relics(run.max_hp, FAIRY_HEAL_PERCENT * multiplier, &run.relics)?;
        ...
        if run.relics.contains(&Relic::LizardTail) && !run.lizard_tail_used {
            run.hp = revival_hp_with_relics(run.max_hp, LIZARD_TAIL_HEAL_PERCENT, &run.relics)?;
...
fn moai_choices(run: &RunState, stage: u32) -> Vec<EventChoice> {
    ...
    let mut choices = vec![EventChoice {
        label: format!(
            ...
            rounded_event_percent(run.max_hp, if run.ascension >= 15 { 0.18 } else { 0.125 })
        ),
    }];
...
fn winding_halls_choices(run: &RunState, stage: u32) -> Vec<EventChoice> {
    ...
    let hp_loss = rounded_event_percent(run.max_hp, if run.ascension >= 15 { 0.18 } else { 0.125 });
    let heal = rounded_event_percent(run.max_hp, if run.ascension >= 15 { 0.20 } else { 0.25 });
    let max_hp_loss = rounded_event_percent(run.max_hp, 0.05);
    vec![
        EventChoice {
...
fn woman_in_blue_choices(run: &RunState) -> Vec<EventChoice> {
    ...
    choices.push(EventChoice {
        label: if run.ascension >= 15 {
            format!(
                "Get punched ({} HP)",
                woman_in_blue_punch_hp_loss(run.max_hp)
            )
        } else {
            "Leave".to_owned()
        },
...
fn neow_option_choices(run: &RunState) -> Vec<EventChoice> {
    generate_neow_options(run.event_rng_seed as i64, run.max_hp)
        .into_iter()
...
fn special_one_time_event_is_available(run: &RunState, event: Event) -> bool {
    match event {
        ...
        Event::KnowingSkull => run.current_act == 2 && run.hp > 12,
...
fn event_is_available(run: &RunState, event: Event) -> bool {
    match event {
        ...
        Event::MoaiHead => {
            has_relic_key(run, Relic::GoldenIdol) || (run.hp as f32 / run.max_hp as f32) <= 0.5
        }
...
pub fn event_screen_for_run(run: &RunState, event: Event) -> EventScreen {
    match event {
        ...
        Event::Ghosts => make_event_screen(event, ghosts_choices(0, run.max_hp), 0),
...
fn enter_event_combat(run: &mut RunState, definitions: &[&MonsterDefinition]) -> SimResult<()> {
    ...
    let mut combat = CombatState::new_run_entry(
        PlayerState::new_run_entry(run.hp, run.max_hp, run.energy_per_turn)?,
        monsters,
        piles,
        run.relics.clone(),
        run.ascension,
...
mod tests {
    ...
    fn pending_obtain_imports_require_exact_event_authority() {
        ...
        low_hp.event = Some(EventScreen {
            ...
            choices: golden_idol_choices(2, low_hp.max_hp, low_hp.ascension),
            stage: 2,
            event_data: 0,
        });
        low_hp
    ...
    fn golden_idol_omamori_consumption_allows_empty_injury_obtain() {
        ...
        run.event = Some(EventScreen {
            event: Event::GoldenIdol,
            choices: golden_idol_choices(1, run.max_hp, run.ascension),
            stage: 1,
            event_data: 0,
        });
    ...
    fn all_three_keys_route_spire_heart_door_into_the_act_four_map() {
        ...
        assert_eq!(act_four.hp, act_four.max_hp);
    ...
    fn neow_curse_gold_defers_curse_obtain_until_leave() {
        ...
        let option_index = generate_neow_options(run.event_rng_seed as i64, run.max_hp)
            .iter()
            .position(|option| {
    ...
    fn neow_option_enters_leave_stage_before_hp_changing_grid_result() {
        ...
        assert_eq!(next.max_hp, 9_000);
        assert_eq!(next.hp, 9_000);
    ...
    fn neow_curse_transform_grid_snapshots_before_curse_obtain() {
        ...
        let options = generate_neow_options(run.event_rng_seed as i64, run.max_hp);
    ...
    fn world_of_goop_hp_loss_uses_tungsten_rod_mitigation() {
        ...
        assert_eq!(next.hp, 90, "Tungsten Rod reduces 11 HP loss to 10");
    ...
    fn lizard_tail_revives_lethal_event_hp_and_keeps_the_leave_screen() {
        ...
        let expected_hp = revival_hp_with_relics(run.max_hp, LIZARD_TAIL_HEAL_PERCENT, &run.relics)
    ...
    fn fairy_revives_lethal_event_hp() {
        ...
        let expected_hp = revival_hp_with_relics(run.max_hp, FAIRY_HEAL_PERCENT, &run.relics)
    ...
    fn bonfire_elementals_removes_rare_card_and_applies_reward() {
        ...
        assert_eq!(after_confirm.max_hp, 80);
        assert_eq!(after_confirm.hp, 80);
    ...
    fn knowing_skull_full_potion_belt_still_consumes_reward_rng() {
        ...
        assert_eq!(after_reward.hp, initial_hp - KNOWING_SKULL_STARTING_COST);
    ...
    fn knowing_skull_success_appends_uncommon_colorless_and_stays_open() {
        ...
        assert_eq!(after.hp, initial_hp - KNOWING_SKULL_STARTING_COST);
    ...
    fn golden_idol_queues_injury_until_obtain_effect_resolves() {
        ...
        run.event = Some(EventScreen {
            event: Event::GoldenIdol,
            choices: golden_idol_choices(0, run.max_hp, run.ascension),
            stage: 0,
            event_data: 0,
        });
    ...
    fn fountain_of_cleansing_drink_applies_parasite_max_hp_loss() {
        ...
        assert!(!after_drink
            ...
            .iter()
            .any(|card| card.content_id == PARASITE_ID));
        assert_eq!(after_drink.max_hp, 9997);
        assert_eq!(after_drink.hp, 7819);
    }
    ...
    fn ghosts_accept_queues_apparitions_until_leave_screen() {
        ...
        run.event = Some(EventScreen {
            event: Event::Ghosts,
            choices: ghosts_choices(0, run.max_hp),
            stage: 0,
            event_data: 0,
        });
    ...
    fn moai_head_accept_heals_after_max_hp_loss() {
        ...
        assert_eq!(after_accept.max_hp, 70);
        assert_eq!(after_accept.hp, 70);
    ...
    fn mark_of_bloom_blocks_run_and_combat_healing() {
        ...
        assert!(combat.has_mark_of_bloom());
