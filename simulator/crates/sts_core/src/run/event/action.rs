...
pub fn apply_event_action(run: &RunState, action: EventAction) -> SimResult<RunState> {
    ...
    apply_validated_event_action(run.clone(), action)
}

pub(crate) fn apply_validated_event_action(
    mut next: RunState,
    action: EventAction,
) -> SimResult<RunState> {
    let previous_phase = next.phase;
    ...
    if previous_phase == RunPhase::Event && next.phase != RunPhase::Event {
