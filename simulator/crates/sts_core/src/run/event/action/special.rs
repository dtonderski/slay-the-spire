...
pub(super) fn apply_special_event_action(
    next: &mut RunState,
    screen: &EventScreen,
    choice_index: usize,
) -> SimResult<bool> {
    match screen.event {
        Event::BonfireElementals => match screen.stage {
            ...
            1 if choice_index == 0 => {
                open_bonfire_elementals_grid(next);
        ...
        Event::Designer => match screen.stage {
            ...
            1 if choice_index == 0 => {
                ...
                if designer_event_data_adjustment_upgrades_one(screen.event_data) {
                    open_event_upgrade_return_to_event_grid(next, Event::Designer);
                } else {
            ...
            1 if choice_index == 1 => {
                ...
                if designer_event_data_cleanup_removes_cards(screen.event_data) {
                    ...
                    next.gold -= cost;
                    open_event_remove_return_to_event_grid(next, Event::Designer);
                } else {
                    if card_count < 2 {
                        return Err(SimError::IllegalAction(
                    ...
                    next.gold -= cost;
                    open_event_transform_return_to_event_grid(next, Event::Designer, 2);
                }
            }
            1 if choice_index == 2 => {
                ...
                next.gold -= cost;
                open_designer_remove_and_upgrade_grid(next);
            }
            1 if choice_index == 3 => {
        ...
        Event::Duplicator => match screen.stage {
            0 if choice_index == 0 => {
                open_duplicator_card_grid(next)?;
                if next.card_grid.is_none() {
                    next.event = Some(make_event_screen(
                        Event::Duplicator,
        ...
        Event::TheWomanInBlue if screen.stage == 0 && choice_index == 3 => {
            if next.ascension >= 15 {
                let hp_loss = woman_in_blue_punch_hp_loss(next.max_hp);
                lose_event_hp(next, hp_loss);
            }
            next.event = Some(make_event_screen(
                Event::TheWomanInBlue,
        ...
        Event::FaceTrader => match screen.stage {
            ...
            1 if choice_index == 0 => {
                let damage = face_trader_damage(next.max_hp);
                next.gain_gold(face_trader_gold(next.ascension))?;
                lose_event_hp(next, damage);
                next.event = Some(EventScreen {
                    event: Event::FaceTrader,
            ...
            1 if choice_index == 1 => {
                let key = roll_face_trader_relic(next);
                next.gain_relic_key(key)?;
                next.event = Some(EventScreen {
                    event: Event::FaceTrader,
                    choices: face_trader_choices(2),
        ...
        Event::NoteForYourself => match screen.stage {
            ...
            1 if choice_index == 0 => {
                ...
                open_event_remove_return_to_event_grid(next, Event::NoteForYourself);
            }
            1 if choice_index == 1 => {
                next.event = Some(EventScreen {
                    event: Event::NoteForYourself,
        ...
        Event::KnowingSkull => match screen.stage {
            ...
            1 if choice_index == 0 => {
                ...
                knowing_skull_gain_random_potion(next);
                next.event = Some(EventScreen {
                    event: Event::KnowingSkull,
                    choices: knowing_skull_choices(1, event_data),
                    stage: 1,
            ...
            1 if choice_index == 2 => {
                ...
                costs.card += 1;
                let event_data = knowing_skull_event_data(costs)?;
                knowing_skull_gain_random_colorless(next)?;
                next.event = Some(EventScreen {
                    event: Event::KnowingSkull,
                    choices: knowing_skull_choices(1, event_data),
                    stage: 1,
