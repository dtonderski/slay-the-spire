...
pub(super) fn apply_act_two_event_action(
    ...
    match screen.event {
        ...
        Event::Vampires if screen.stage == 0 && choice_index == 0 => {
            let loss = vampires_max_hp_loss(next.max_hp);
        ...
        Event::ForgottenAltar => match screen.stage {
            ...
            0 if choice_index == usize::from(next.relics.contains(&Relic::GoldenIdol)) => {
                let hp_loss = forgotten_altar_hp_loss(next.max_hp, next.ascension);
        ...
        Event::Ghosts => match screen.stage {
            0 if choice_index == 0 => {
                let loss = ghosts_max_hp_loss(next.max_hp);
                next.max_hp = (next.max_hp - loss).max(1);
                next.hp = next.hp.min(next.max_hp);
                ...
                next.event = Some(EventScreen {
                    event: Event::Ghosts,
                    choices: ghosts_choices(1, next.max_hp),
                    stage: 1,
