...
pub(super) fn apply_act_one_event_action(
    ...
    match screen.event {
        Event::GoldenIdol => match screen.stage {
            0 if choice_index == 0 => {
                ...
                next.event = Some(EventScreen {
                    ...
                    choices: golden_idol_choices(1, next.max_hp, next.ascension),
                    stage: 1,
                    event_data: 0,
                });
            }
            0 if choice_index == 1 => {
                next.event = Some(EventScreen {
                    event: Event::GoldenIdol,
                    choices: golden_idol_choices(3, next.max_hp, next.ascension),
                    stage: 3,
                    event_data: 0,
                });
            }
            1 if choice_index == 0 => {
                ...
                next.event = Some(EventScreen {
                    event: Event::GoldenIdol,
                    choices: golden_idol_choices(2, next.max_hp, next.ascension),
                    stage: 2,
                    event_data: 0,
                });
            }
            1 if choice_index == 1 => {
                let hp_loss = golden_idol_hp_loss(next.max_hp, next.ascension);
            ...
            1 if choice_index == 2 => {
                let max_hp_loss = golden_idol_max_hp_loss(next.max_hp, next.ascension);
                next.max_hp = (next.max_hp - max_hp_loss).max(1);
        ...
        Event::ShiningLight if screen.stage == 0 && choice_index == 0 => {
            let loss = shining_light_hp_loss(next.max_hp);
