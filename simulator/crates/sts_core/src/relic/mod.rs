...
pub fn apply_shuffle_relics(state: &mut CombatState) -> SimResult<Vec<InternalAction>> {
    ...
    if state.player.authority.relics.contains(&Relic::TheAbacus) {
    ...
    if state.player.authority.relics.contains(&Relic::Sundial) {
        state.sundial_shuffles = state.sundial_shuffles.wrapping_add(1);
        if state.sundial_shuffles.is_multiple_of(SUNDIAL_THRESHOLD) {
...
pub fn monster_death_relic_actions(state: &CombatState) -> Vec<crate::action::InternalAction> {
    if state.player.authority.relics.contains(&Relic::GremlinHorn) {
        vec![
...
pub fn apply_monster_death_relics(state: &mut CombatState) -> SimResult<()> {
    ...
    if next.player.authority.relics.contains(&Relic::GremlinHorn) {
...
pub fn heal_combat_player_with_relics(state: &mut CombatState, base_heal: i32) -> SimResult<()> {
    if state.has_mark_of_bloom() {
        return Ok(());
    }

    let mut next = state.clone();
    let max_hp = next.player.max_hp;
    let relics = next.relics.clone();
    heal_player_in_combat_with_relics(&mut next.player.hp, max_hp, base_heal, &relics);
...
pub fn apply_player_hp_loss_relics_with_draw_policy(
    ...
    if next
        ...
        .authority
        .relics
        .contains(&Relic::CentennialPuzzle)
    ...
    if next
        ...
        .authority
        .relics
        .contains(&Relic::SelfFormingClay)
    {
    ...
    if next.player.authority.relics.contains(&Relic::RunicCube) {
...
pub fn settle_deferred_hp_loss_draw_relics(
    ...
    if runic_draws > 0 && next.player.authority.relics.contains(&Relic::RunicCube) {
...
pub fn sync_red_skull_strength(state: &mut CombatState) -> SimResult<()> {
    sync_red_skull_strength_present(
        state,
        state.player.authority.relics.contains(&Relic::RedSkull),
    )
}
...
pub fn apply_start_of_player_turn_relics(state: &mut CombatState) -> SimResult<Vec<i32>> {
    ...
    for relic in state.player.authority.relics.clone() {
    ...
    if state.player.authority.relics.contains(&Relic::HappyFlower) {
        checked_increment_relic_counter(&mut state.happy_flower_turns)?;
        if state.happy_flower_turns >= HAPPY_FLOWER_THRESHOLD {
            state.happy_flower_turns = 0;
            if state.relic_counters.player_turns_started == 1
                && state.player.authority.relics.contains(&Relic::Toolbox)
            {
                checked_add_relic_value(
    ...
    if state.player.authority.relics.contains(&Relic::ArtOfWar)
    ...
    if state
        ...
        .authority
        .relics
        .contains(&Relic::IncenseBurner)
    {
        checked_increment_relic_counter(&mut state.incense_burner_counter)?;
        if state.incense_burner_counter >= INCENSE_BURNER_THRESHOLD {
            state.incense_burner_counter = 0;
...
pub(crate) fn apply_start_of_player_turn_post_draw_relics_deferred_deaths(
    ...
    if state
        ...
        .authority
        .relics
        .contains(&Relic::MercuryHourglass)
    {
        if matches!(
            state.decision,
...
fn has_start_of_turn_relic(state: &CombatState) -> bool {
    state.player.authority.relics.iter().any(|relic| {
        matches!(
            relic,
            Relic::HappyFlower
                | Relic::ArtOfWar
...
pub fn apply_orichalcum_end_of_player_turn(state: &mut CombatState) -> SimResult<()> {
    if state.player.authority.relics.contains(&Relic::Orichalcum) && state.player.block == 0 {
...
pub fn settle_pending_opening_combat_actions(state: &mut CombatState) -> SimResult<()> {
    ...
    if draw_count > 0 {
        ...
        if next.player.authority.relics.contains(&Relic::BagOfMarbles) {
            for monster in next.monsters.iter_mut().filter(|monster| monster.alive) {
                apply_monster_vulnerable_with_relics(
                    &mut monster.powers,
                    &next.player.authority.relics,
                    BAG_OF_MARBLES_VULNERABLE,
                )?;
            }
        }
        ...
        if next.player.authority.relics.contains(&Relic::WarpedTongs) {
...
pub fn apply_on_card_play_relics(
    ...
    if state.player.authority.relics.contains(&Relic::Akabeko) && card_type == CardType::Attack {
        checked_increment_relic_counter(&mut state.relic_counters.attacks_played_this_combat)?;
    }
    if state.player.authority.relics.contains(&Relic::ArtOfWar) && card_type == CardType::Attack {
        checked_increment_relic_counter(&mut state.relic_counters.attacks_played_this_turn)?;
    }

    if state.player.authority.relics.contains(&Relic::InkBottle) {
        checked_increment_relic_counter(&mut state.ink_bottle_cards_played)?;
        if state.ink_bottle_cards_played >= INK_BOTTLE_THRESHOLD {
            state.ink_bottle_cards_played = 0;
    ...
    if state.player.authority.relics.contains(&Relic::Nunchaku) && card_type == CardType::Attack {
        checked_increment_relic_counter(&mut state.nunchaku_attacks_played)?;
        if state.nunchaku_attacks_played >= NUNCHAKU_THRESHOLD {
            state.nunchaku_attacks_played = 0;
            follow_ups.push(InternalAction::GainEnergy {
    ...
    if state.player.authority.relics.contains(&Relic::PenNib) && card_type == CardType::Attack {
        checked_increment_relic_counter(&mut state.pen_nib_attacks_played)?;
        if state.pen_nib_attacks_played >= PEN_NIB_THRESHOLD {
            state.pen_nib_attacks_played = 0;
    ...
    if state.player.authority.relics.contains(&Relic::Shuriken) && card_type == CardType::Attack {
    ...
    if state.player.authority.relics.contains(&Relic::Kunai) && card_type == CardType::Attack {
    ...
    if state.player.authority.relics.contains(&Relic::LetterOpener) && card_type == CardType::Skill
    {
    ...
    if state.player.authority.relics.contains(&Relic::BirdFacedUrn) && card_type == CardType::Power
    {
...
pub fn can_play_card_with_relics(state: &CombatState) -> bool {
    !state.player.authority.relics.contains(&Relic::VelvetChoker)
...
mod tests {
    ...
    fn orange_pellets_type_flags_reset_each_turn_and_do_not_cross_cleanse() {
        ...
        state.player.authority.relics.push(Relic::OrangePellets);
    ...
    fn warped_tongs_upgrades_before_later_pocketwatch_draws() {
        ...
        let mut state = CombatState::initial_fixture();
        state.player.authority.relics = vec![Relic::WarpedTongs, Relic::Pocketwatch];
