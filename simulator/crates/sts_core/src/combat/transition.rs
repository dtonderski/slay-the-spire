...
mod tests {
    ...
    use crate::content::cards::{
        ...
        DARK_SHACKLES_PLUS_ID, DEEP_BREATH_ID, DEFEND_R_ID, DEFEND_R_PLUS_ID, DEMON_FORM_ID,
        DEMON_FORM_PLUS_ID, DISARM_ID, DISCOVERY_ID, DOUBLE_TAP_ID, DOUBLE_TAP_PLUS_ID,
        DROPKICK_ID, DUAL_WIELD_ID, ENLIGHTENMENT_ID, ENLIGHTENMENT_PLUS_ID, ENTRENCH_ID,
        EVOLVE_ID, EXHUME_ID, EXHUME_PLUS_ID, FEED_ID, FEED_PLUS_ID, FEEL_NO_PAIN_ID,
        FIEND_FIRE_ID, FIEND_FIRE_PLUS_ID, FINESSE_ID, FIRE_BREATHING_ID, FLAME_BARRIER_ID,
        FLASH_OF_STEEL_ID, FLEX_ID, FLEX_PLUS_ID, FORETHOUGHT_ID, FORETHOUGHT_PLUS_ID,
        GHOSTLY_ARMOR_ID, GOOD_INSTINCTS_ID, HAND_OF_GREED_ID, HAVOC_ID, HEADBUTT_ID,
        HEADBUTT_PLUS_ID, HEAVY_BLADE_ID, HEAVY_BLADE_PLUS_ID, HEMOKINESIS_ID, IMMOLATE_ID,
    ...
    use crate::content::monsters::{
        ...
        GREMLIN_WARRIOR_A0, THE_COLLECTOR_A0, TORCH_HEAD_A0,
    };
    ...
    #[test]
    fn killing_collector_makes_living_torch_heads_escape() {
        let mut state = CombatState::initial_fixture();
        state.monsters = vec![
            monster_state(&TORCH_HEAD_A0, MonsterId::new(1)),
            monster_state(&TORCH_HEAD_A0, MonsterId::new(2)),
            monster_state(&THE_COLLECTOR_A0, MonsterId::new(3)),
        ];
        state.monsters[0].powers.minion = 1;
        state.monsters[1].powers.minion = 1;
        state.monsters[2].hp = 6;
    ...
    #[test]
    fn defend_plus_increases_player_block_by_eight() {
        let state = hand_only(DEFEND_R_PLUS_ID);

        let next = apply_combat_action(
            &state,
            CombatAction::PlayCard {
                card_id: hand_card_id(&state, DEFEND_R_PLUS_ID),
                target: None,
            },
        )
        .expect("Defend+ applies");

        assert_eq!(next.player.block, state.player.block + 8);
    }
    ...
    #[test]
    fn heavy_blade_plus_applies_strength_five_times() {
        let mut state = hand_only(HEAVY_BLADE_PLUS_ID);
        state.player.powers.strength = 2;

        let next = apply_combat_action(
            &state,
            CombatAction::PlayCard {
                card_id: hand_card_id(&state, HEAVY_BLADE_PLUS_ID),
    ...
    #[test]
    fn secret_weapon_opens_draw_select_for_draw_pile_attacks() {
        ...
        let after_play = apply_combat_action(&state, secret_weapon_action(&state))
            .expect("Secret Weapon opens draw select");

        assert_eq!(after_play.player.energy, state.player.energy);
        assert_eq!(
            after_play.draw_select.as_ref().map(|select| select.purpose),
            Some(DrawSelectPurpose::SecretWeaponAttackToHand)
        );
        assert_eq!(draw_select_ui_to_draw_index(&after_play, 0), Ok(1));
        assert_eq!(draw_select_ui_to_draw_index(&after_play, 1), Ok(2));
        assert_eq!(
        ...
        assert_eq!(after_play.piles.hand[0].content_id, SECRET_WEAPON_ID);
    }

    #[test]
    fn secret_weapon_confirm_moves_selected_attack_to_hand_and_exhausts() {
        let mut state = hand_only(SECRET_WEAPON_ID);
        state.piles.draw_pile = vec![
            CardInstance::new(CardId::new(30), DEFEND_R_ID),
            CardInstance::new(CardId::new(31), STRIKE_R_ID),
        ...
        let mut after_play = apply_combat_action(&state, secret_weapon_action(&state))
            .expect("Secret Weapon opens draw select");
        choose_draw_select(&mut after_play, 1).expect("choose Bash");
    ...
    #[test]
    fn secret_weapon_without_draw_pile_attacks_is_illegal() {
        let mut state = hand_only(SECRET_WEAPON_ID);
        state.piles.draw_pile = vec![CardInstance::new(CardId::new(30), DEFEND_R_ID)];

        assert_eq!(
            apply_combat_action(&state, secret_weapon_action(&state)),
            Err(SimError::IllegalAction(
    ...
    fn discovery_opens_three_card_reward_and_exhausts_source() {
        ...
        while expected.len() < 3 {
            ...
            if !expected.contains(&content_id) {
    ...
    #[test]
    fn discovery_without_card_random_rng_uses_deterministic_modeled_fallback() {
        let state = hand_only(DISCOVERY_ID);
        let expected = discovery_modeled_card_pool()
            .into_iter()
            .take(3)
            .collect::<Vec<_>>();
    ...
    fn discovery_event_log_records_choice_open_before_source_exhaust() {
        ...
        let transition = apply_combat_action_with_events(&state, discovery_action(&state))
    ...
    fn feed_fatal_damage_increases_current_and_max_hp_by_three() {
        ...
        assert_eq!(next.player.max_hp, 74);
    ...
    #[test]
    fn shockwave_does_not_reduce_monster_outgoing_attack_damage() {
    ...
    #[test]
    fn anger_plus_deals_eight_damage_and_copies_upgraded_card() {
    ...
    #[test]
    fn top_draw_reaper_damages_all_living_enemies_and_heals() {
        let mut state = two_monster_hand(STRIKE_R_ID);
        state.player.hp = 50;
        state.monsters[0].block = 2;
        state.piles.draw_pile = vec![CardInstance::new(CardId::new(30), REAPER_ID)];

        let next = apply_play_top_draw_card_action(&state, None).expect("top Reaper applies");

        assert_eq!(next.monsters[0].hp, state.monsters[0].hp - 2);
        assert_eq!(next.monsters[1].hp, state.monsters[1].hp - 4);
        assert_eq!(next.player.hp, 56);
        assert!(next.piles.draw_pile.is_empty());
        assert_eq!(next.piles.exhaust_pile[0].content_id, REAPER_ID);
    }
    ...
    #[test]
    fn reaper_heals_actual_hp_damage_not_overkill() {
        let mut state = two_monster_hand(REAPER_ID);
        state.player.hp = 50;
    ...
    fn burning_pact_exhausts_lowest_other_hand_card_and_draws_two() {
        ...
        confirm_exhaust_select(&mut next).expect("confirms Burning Pact select");
    ...
    fn burning_pact_exhausting_sentinel_grants_two_energy() {
        ...
        .expect("Burning Pact applies");
        choose_exhaust_select(&mut next, 0).expect("chooses Sentinel");
        confirm_exhaust_select(&mut next).expect("confirms Burning Pact select");
    ...
    fn burning_pact_exhausting_non_sentinel_does_not_grant_energy() {
        ...
        .expect("Burning Pact applies");
        choose_exhaust_select(&mut next, 0).expect("chooses Defend");
        confirm_exhaust_select(&mut next).expect("confirms Burning Pact select");
    ...
    #[test]
    fn feel_no_pain_grants_power_and_is_removed_from_hand() {
        let mut state = CombatState::initial_fixture();
    ...
    #[test]
    fn dark_embrace_grants_power_and_is_removed_from_hand() {
        let mut state = CombatState::initial_fixture();
    ...
    #[test]
    fn demon_form_plus_grants_three_ritual() {
        let mut state = CombatState::initial_fixture();
        state.player.energy = 3;
        state.piles.hand = vec![CardInstance::new(CardId::new(20), DEMON_FORM_PLUS_ID)];
    ...
    fn sadistic_nature_triggers_for_weak_vulnerable_and_strength_down_actions() {
        ...
        assert_eq!(next.monsters[0].powers.vulnerable, 3);
        assert_eq!(next.monsters[0].powers.strength, 0);
        assert_eq!(next.monsters[0].hp, monster_hp - 10);
    }
    ...
    #[test]
    fn double_tap_replayed_attack_counts_as_extra_card_play() {
        let mut state = hand_only(STRIKE_R_ID);
        state.double_tap_pending = 1;
    ...
    #[test]
    fn sword_boomerang_queues_malleable_block_after_all_three_hits() {
        let mut state = hand_only(SWORD_BOOMERANG_ID);
        state.player.powers.strength = 6;
        state.player.powers.weak = 1;
        state.monsters[0].hp = 42;
        ...
        assert_eq!(next.monsters[0].hp, 15);
        assert_eq!(next.monsters[0].block, 15);
    ...
    #[test]
    fn pommel_strike_plus_deals_ten_damage_and_draws_two() {
        let mut state = hand_only(POMMEL_STRIKE_PLUS_ID);
        state.piles.draw_pile = vec![
            CardInstance::new(CardId::new(30), STRIKE_R_ID),
            CardInstance::new(CardId::new(31), DEFEND_R_ID),
        ];
    ...
    #[test]
    fn battle_trance_draws_three_sets_cannot_draw_and_moves_to_discard() {
        let mut state = CombatState::initial_fixture();
    ...
    #[test]
    fn battle_trance_plus_draws_four_cards() {
        let mut state = CombatState::initial_fixture();
    ...
    #[test]
    fn bloodletting_loses_three_hp_and_gains_three_energy() {
        let mut state = CombatState::initial_fixture();
        state.player.energy = 0;
    ...
    fn battle_trance_blocks_later_draw_from_shrug_it_off() {
        ...
        state.piles.draw_pile = vec![
            CardInstance::new(CardId::new(33), ANGER_ID),
            CardInstance::new(CardId::new(32), BASH_ID),
            CardInstance::new(CardId::new(30), STRIKE_R_ID),
            CardInstance::new(CardId::new(31), DEFEND_R_ID),
        ];
        ...
        assert_eq!(after_shrug.piles.hand.len(), 3);
        assert!(!after_shrug
            .piles
            .hand
            .iter()
            .any(|card| card.id == CardId::new(33)));
    }
    ...
    #[test]
    fn gremlin_nob_enrage_does_not_trigger_before_bellow() {
        let state = CombatState::gremlin_nob_fixture();
    ...
    fn gremlin_nob_enrage_stacks_on_multiple_skills() {
        ...
        assert_eq!(after_second.monsters[0].powers.anger, 2);
    ...
    #[test]
    fn armaments_confirm_exhausts_source_under_corruption() {
        let mut state = hand_only(ARMAMENTS_ID);
        state.player.energy = 0;
        state.player.powers.corruption = 1;
        state.piles.hand = vec![
        ...
        .expect("Armaments opens hand select under Corruption");
        ...
        assert_eq!(after_play.player.energy, 0);
        assert!(after_play.piles.discard_pile.is_empty());
        assert_eq!(after_play.piles.exhaust_pile[0].content_id, ARMAMENTS_ID);
        assert_eq!(after_play.piles.hand[0].content_id, STRIKE_R_PLUS_ID);
    }

    #[test]
    fn armaments_confirm_returns_selected_card_to_end_of_hand() {
        let mut state = hand_only(ARMAMENTS_ID);
        state.piles.hand = vec![
            CardInstance::new(CardId::new(25), REGRET_ID),
            CardInstance::new(CardId::new(21), STRIKE_R_ID),
            CardInstance::new(CardId::new(22), DEFEND_R_ID),
            CardInstance::new(CardId::new(23), STRIKE_R_ID),
            CardInstance::new(CardId::new(20), ARMAMENTS_ID),
        ...
        choose_hand_select(&mut after_play, 0).expect("choose first Strike");
        confirm_hand_select(&mut after_play).expect("confirm Armaments select");
        ...
        assert_eq!(
            after_play
                ...
                .collect::<Vec<_>>(),
            vec![
                (CardId::new(22), DEFEND_R_ID),
                (CardId::new(23), STRIKE_R_ID),
                (CardId::new(24), STRIKE_R_ID),
                (CardId::new(21), STRIKE_R_PLUS_ID),
                (CardId::new(25), REGRET_ID),
            ]
        );
    }

    #[test]
    fn armaments_confirm_matches_trace_order_with_hidden_regret_and_middle_source() {
        let mut state = hand_only(ARMAMENTS_ID);
        state.piles.hand = vec![
            CardInstance::new(CardId::new(25), REGRET_ID),
            CardInstance::new(CardId::new(20), ARMAMENTS_ID),
            CardInstance::new(CardId::new(21), DEFEND_R_ID),
            CardInstance::new(CardId::new(22), STRIKE_R_ID),
            CardInstance::new(CardId::new(23), CLASH_ID),
        ];
        ...
        choose_hand_select(&mut after_play, 1).expect("choose Strike");
        confirm_hand_select(&mut after_play).expect("confirm Armaments select");
        ...
        assert_eq!(
            after_play
                ...
                .collect::<Vec<_>>(),
            vec![
                (CardId::new(21), DEFEND_R_ID),
                (CardId::new(23), CLASH_ID),
                (CardId::new(22), STRIKE_R_PLUS_ID),
                (CardId::new(25), REGRET_ID),
            ]
        );
    }
    ...
    #[test]
    fn dual_wield_opens_hand_select_then_copies_attack_to_hand_and_exhausts() {
        ...
        assert_eq!(
            next.hand_select.as_ref().map(|select| select.purpose),
            Some(HandSelectPurpose::DualWieldCopy)
        );
        assert_eq!(hand_select_ui_to_hand_index(&next, 0), Ok(2));
        assert_eq!(hand_select_ui_to_hand_index(&next, 1), Ok(3));
        choose_hand_select(&mut next, 0).expect("choose Anger");
        confirm_hand_select(&mut next).expect("confirm Dual Wield select");
        ...
        assert_eq!(
            next.piles
                ...
                .map(|card| card.content_id)
                .collect::<Vec<_>>(),
            vec![STRIKE_R_ID, DEFEND_R_ID, ANGER_ID, ANGER_ID]
        );
    ...
    #[test]
    fn uppercut_plus_artifact_blocks_weak_before_vulnerable() {
        let mut state = hand_only(UPPERCUT_PLUS_ID);
        state.relics.push(Relic::ChampionBelt);
        state.monsters[0].powers.artifact = 1;
    ...
    fn fiend_fire_event_log_records_local_exhaust_then_damage_order() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                InternalAction::SpendEnergy { amount: 2 },
                InternalAction::ExhaustRandomHandCardExcept {
                    excluded_card_id: CardId::new(20),
                },
                InternalAction::CardExhausted {
                    card_id: CardId::new(21)
                },
    ...
    fn discovery_action(state: &CombatState) -> CombatAction {
        CombatAction::PlayCard {
            card_id: hand_card_id(state, DISCOVERY_ID),
            target: None,
        }
    }
    ...
    fn secret_weapon_action(state: &CombatState) -> CombatAction {
        CombatAction::PlayCard {
            card_id: hand_card_id(state, SECRET_WEAPON_ID),
            target: None,
        }
    }
