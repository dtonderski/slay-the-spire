...
#[cfg(test)]
thread_local! {
    static PUBLIC_TRANSACTION_CLONE_COUNT: std::cell::Cell<usize> = const { std::cell::Cell::new(0) };
}

#[cfg(test)]
fn reset_public_transaction_clone_count() {
    PUBLIC_TRANSACTION_CLONE_COUNT.with(|count| count.set(0));
}

#[cfg(test)]
fn public_transaction_clone_count() -> usize {
    PUBLIC_TRANSACTION_CLONE_COUNT.with(std::cell::Cell::get)
}
...
pub fn apply_combat_action(state: &CombatState, action: CombatAction) -> SimResult<CombatState> {
    Ok(apply_combat_action_impl(state, action, false)?.state)
}
...
pub fn apply_combat_action_with_events(
    ...
    apply_combat_action_impl(state, action, true)
}

fn apply_combat_action_impl(
    state: &CombatState,
    action: CombatAction,
    record_events: bool,
) -> SimResult<CombatTransition> {
    ...
    #[cfg(test)]
    PUBLIC_TRANSACTION_CLONE_COUNT.with(|count| count.set(count.get() + 1));
    apply_validated_combat_action_owned(working, action, record_events)
}

pub(crate) fn apply_validated_combat_action_owned(
    working: CombatState,
    action: CombatAction,
    record_events: bool,
) -> SimResult<CombatTransition> {
    let mut transition = match action {
        CombatAction::PlayCard { card_id, target } => {
            apply_play_card(working, card_id, target, record_events)
        }
        CombatAction::EndTurn => apply_end_turn(working, record_events),
    }?;
...
fn apply_end_turn(mut state: CombatState, record_events: bool) -> SimResult<CombatTransition> {
    if state.opening_turn_pending {
        ...
        let relics = next.player.authority.relics.clone();
    ...
    let ethereal_ids = record_events.then(|| end_turn_ethereal_hand_card_ids(&state));
    ...
    let next = crate::combat::turn::end_player_turn_owned(state)?;
...
fn apply_play_card(
    ...
    let queue = card_effects::play_card_queue_in_place(&mut prepared, card_id, target)?;
...
pub(crate) fn process_internal_queue(
    ...
    process_internal_queue_with_event_recording(state, queue, false)
}

fn process_internal_queue_with_event_recording(
    state: &CombatState,
    queue: VecDeque<InternalAction>,
    record_events: bool,
) -> SimResult<CombatTransition> {
    process_internal_queue_owned(state.clone(), queue, record_events)
}
...
fn process_internal_queue_owned(
                ...
                Some(CombatDecisionState::HandSelect { .. }) => {
                    ...
                    while let Some(queued) = queue.pop_front() {
                        if matches!(
                            ...
                            record_event(&mut event_log, queued);
        ...
        && next.decision.is_none()
    {
        ...
        if next.monsters.iter().any(|monster| monster.alive) {
            next = crate::combat::turn::end_player_turn_owned(next)?;
        } else {
...
#[inline]
fn record_event(event_log: &mut Option<Vec<InternalAction>>, action: InternalAction) {
    if let Some(event_log) = event_log {
        event_log.push(action);
    }
}
...
pub fn flush_pending_player_spikes_damage_if_ready(state: &mut CombatState) -> SimResult<()> {
    ...
    let hp_loss = reflect_spikes_to_player(&mut next.player, &relics, damage);
...
fn apply_monster_death_hooks_deferred_relics(
    ...
    if combat_continues_after_monster_death(&next)
        && next.player.authority.relics.contains(&Relic::GremlinHorn)
    {
...
pub(super) fn apply_player_vulnerable_debuff(
    ...
    let applies_champion_belt = state
        .player
        .authority
        .relics
        .contains(&crate::Relic::ChampionBelt);
...
pub(crate) fn apply_juggernaut_random_damage(
    ...
    let hand_drill = state
        .player
        .authority
        .relics
        .contains(&crate::Relic::HandDrill);
...
pub(crate) fn reserve_dead_branch_card_content(state: &mut CombatState) -> Option<ContentId> {
    if !state.player.authority.relics.contains(&Relic::DeadBranch)
        || !combat_continues_after_monster_death(state)
    {
        return None;
    }
...
fn apply_on_exhaust_effects_inner(
    ...
    if state.player.authority.relics.contains(&Relic::CharonsAshes) {
        ...
        let hand_drill = state.player.authority.relics.contains(&Relic::HandDrill);
        let targets = state
            .monsters
            .iter()
            .filter(|monster| monster.alive)
    ...
    while !next.deferred_mayhem_play_top_settlements.is_empty() {
        ...
        if destination == CardPile::ExhaustPile
            && next
                ...
                .authority
                .relics
                .contains(&crate::Relic::StrangeSpoon)
            && !next
...
fn resolve_top_draw_card(
        ...
        || !crate::relic::can_play_card_with_relics(state)
    {
        ...
        if exhaust_played_card || definition.keywords.exhaust {
            ...
            let spoon_saves = state.player.authority.relics.contains(&Relic::StrangeSpoon)
...
pub fn settle_time_warp_pre_discard_if_ready_public(state: &mut CombatState) -> SimResult<()> {
        ...
        && state.decision.is_none()
    {
        ...
        crate::combat::hand::discard_end_of_turn_hand(state)?;
        if hand_nonempty_at_end_click
            && !state
                .player
                .authority
                .relics
                .contains(&crate::Relic::RunicPyramid)
        {
...
fn move_delayed_played_source_with_bot_exhaust_queue(
    ...
    let generated = state.player.authority.relics.contains(&Relic::DeadBranch)
        && state.monsters.iter().any(|monster| monster.alive);
...
fn forced_source_card_destination(
    ...
    if state.player.authority.relics.contains(&Relic::StrangeSpoon)
        && state.rng.card_random_rng.random_bool()
    {
        CardPile::DiscardPile
    } else {
...
fn force_exhaust_armaments_source(
    ...
    let queue_bot_exhaust_follow_ups = state.player.authority.relics.contains(&Relic::DeadBranch)
...
mod tests {
    ...
    fn no_event_path_matches_recording_path() {
        ...
        let action = legal_combat_actions(&state)
            .expect("fixture is valid")
            .into_iter()
            .find(|action| matches!(action, CombatAction::PlayCard { .. }))
            .expect("fixture has a playable card");

        reset_public_transaction_clone_count();
        let without_events = apply_combat_action(&state, action).expect("action succeeds");
        assert_eq!(public_transaction_clone_count(), 1);

        reset_public_transaction_clone_count();
        let with_events = apply_combat_action_with_events(&state, action).expect("action succeeds");
        assert_eq!(public_transaction_clone_count(), 1);
    ...
    fn deferred_mayhem_spoon_roll_follows_residual_card_random_insert() {
        ...
        let mut state = CombatState::initial_fixture();
        state.player.authority.relics = vec![Relic::StrangeSpoon];
        ...
        let (result, events) = crate::rng::capture_rng_trace(|| {
            flush_deferred_mayhem_play_top_draw_inserts(&mut queued)
        });
        ...
        assert!(matches!(
            events[0].operation,
            crate::rng::RngTraceOperation::RandomInt { .. }
        ));
        assert!(matches!(
            events[1].operation,
            crate::rng::RngTraceOperation::RandomBool { .. }
        ));
    }
    ...
    fn captains_wheel_block_triggers_juggernaut() {
        ...
        state.player.authority.relics.push(Relic::CaptainsWheel);
    ...
    fn havoc_with_ink_bottle_plays_original_top_then_draws_next() {
        let mut state = CombatState::initial_fixture();
        state.player.energy = 1;
        state.player.authority.relics.push(Relic::InkBottle);
        state.ink_bottle_cards_played = INK_BOTTLE_THRESHOLD - 1;
        ...
        assert_eq!(next.ink_bottle_cards_played, 1); // Havoc + Whirlwind
    }
    ...
    fn corruption_empty_draw_havoc_dead_branch_generates_source_card_before_top() {
        ...
        let mut state = CombatState::initial_fixture();
        state.player.energy = 1;
        state.player.authority.relics.push(Relic::DeadBranch);
        state.player.powers.corruption = 1;
        state.piles.hand = vec![CardInstance::new(CardId::new(1), HAVOC_ID)];
        state.piles.draw_pile.clear();
        state.piles.discard_pile = vec![CardInstance::new(CardId::new(2), DEFEND_R_ID)];
    ...
    fn nested_havoc_empty_draw_dark_embrace_playtops_parent_not_defend() {
        ...
        state.player.authority.relics = vec![
            Relic::BurningBlood,
            Relic::NeowsLament,
    ...
    fn red_skull_healing_failure_reaches_the_combat_action_boundary() {
        let mut state = CombatState::initial_fixture();
        state.player.authority.relics.push(Relic::RedSkull);
    ...
    fn pain_loss_triggers_centennial_draw_before_burning_pact_selection() {
        ...
        state.player.hp = 50;
        state.player.max_hp = 50;
        state.player.authority.relics.push(Relic::CentennialPuzzle);
        state.piles.hand = vec![
    ...
    fn champion_belt_weak_overflow_fails_closed_at_the_combat_action_boundary() {
        let mut state = CombatState::initial_fixture();
        state.player.authority.relics.push(Relic::ChampionBelt);
    ...
    fn nunchaku_energy_overflow_fails_closed_at_the_combat_action_boundary() {
        let mut state = CombatState::initial_fixture();
        state.player.energy = i32::MAX;
        state.player.authority.relics.push(Relic::Nunchaku);
        state.nunchaku_attacks_played = crate::relic::NUNCHAKU_THRESHOLD - 1;
    ...
    fn shuffle_trigger_state(relic: Relic) -> CombatState {
        let mut state = CombatState::initial_fixture();
        state.player.authority.relics.push(relic);
    ...
    fn lethal_pommel_draw_skips_empty_deck_shuffle_and_sundial() {
        ...
        let target = MonsterId::new(1);
        let mut state = CombatState::initial_fixture();
        state.player.authority.relics.push(Relic::Sundial);
        state.sundial_shuffles = 2;
    ...
    fn sundial_energy_overflow_fails_closed_at_the_combat_action_boundary() {
        let mut state = shuffle_trigger_state(Relic::Sundial);
        state.player.energy = i32::MAX;
        state.sundial_shuffles = crate::relic::SUNDIAL_THRESHOLD - 1;
    ...
    fn later_gremlin_horn_actions_precede_first_draws_evolve_follow_up() {
        ...
        state.player.authority.relics = vec![Relic::GremlinHorn];
        state.piles.hand.clear();
        state.piles.draw_pile = vec![
            CardInstance::new(CardId::new(1), STRIKE_R_ID),
            CardInstance::new(CardId::new(2), STRIKE_R_ID),
        ...
        let transition = process_internal_queue_with_event_recording(
            &state,
            VecDeque::from([
                InternalAction::ApplyGremlinHornOnDeath,
                InternalAction::ApplyGremlinHornOnDeath,
    ...
    fn violence_strange_spoon_rolls_after_attack_tmp_group() {
        ...
        let mut state = CombatState::initial_fixture();
        state.player.energy = 1;
        state.player.authority.relics.push(Relic::StrangeSpoon);
    ...
    fn feed_kill_applies_magic_flower_to_the_hp_gain() {
        ...
        state.player.energy = 3;
        state.player.authority.relics.push(Relic::MagicFlower);
    ...
    fn feed_kill_with_mark_of_bloom_raises_max_hp_without_healing() {
        ...
        state.player.max_hp = 109;
        state.player.energy = 3;
        state.player.authority.relics.push(Relic::MarkOfBloom);
    ...
    fn double_tap_and_necronomicon_each_copy_the_original_once() {
        ...
        let mut state = CombatState::initial_fixture();
        state.player.authority.relics.push(Relic::Necronomicon);
        state.double_tap_pending = 1;
        state.player.energy = 2;
        state.piles.hand = vec![CardInstance::new(CardId::new(1), HEAVY_BLADE_ID)];
        state.piles.draw_pile.clear();
    ...
    fn fiend_fire_with_dead_branch_spends_energy_and_refills_hand() {
        ...
        state.player.energy = 3;
        state.player.authority.relics = vec![Relic::DeadBranch];
        state.piles.hand = vec![
    ...
    fn double_tap_anger_hand_drill_applies_vulnerable_before_copy() {
        ...
        state.player.authority.relics.push(Relic::HandDrill);
        state.double_tap_pending = 1;
        state.piles.hand = vec![CardInstance::new(CardId::new(1), ANGER_ID)];
    ...
    fn pen_nib_doubles_tenth_single_attack() {
        ...
        state.monsters[0].max_hp = 50;
        state.player.energy = 1;
        state.player.authority.relics.push(Relic::PenNib);
        state.pen_nib_attacks_played = 9;
        state.piles.hand = vec![CardInstance::new(CardId::new(1), STRIKE_R_ID)];
        state.piles.draw_pile.clear();
        state.piles.discard_pile.clear();
        let next = apply_combat_action(
    ...
    fn double_tap_pen_nib_on_second_hit_when_counter_starts_at_eight() {
        ...
        state.double_tap_pending = 1;
        state.player.authority.relics.push(Relic::PenNib);
        state.pen_nib_attacks_played = 8;
        state.piles.hand = vec![CardInstance::new(CardId::new(1), RECKLESS_CHARGE_ID)];
        state.piles.draw_pile.clear();
        state.piles.discard_pile.clear();
        state.piles.exhaust_pile.clear();
    ...
    fn dead_target_necronomicon_fiend_fire_copy_skips_card_play_triggers() {
        ...
        state.player.energy = 3;
        state.player.authority.relics = vec![Relic::Necronomicon, Relic::ArtOfWar];
        state.piles.hand = vec![
    ...
    fn havoc_force_pain_with_blue_candle_loses_hp() {
        ...
        state.player.max_hp = 50;
        state.player.powers.rupture = 1;
        state.player.authority.relics.push(Relic::BlueCandle);
        state.piles.hand = vec![CardInstance::new(CardId::new(1), HAVOC_PLUS_ID)];
    ...
    fn dual_havoc_doubt_dead_branch_probe() {
        ...
        let seed = 34_961_238_619_619i64.wrapping_add(44);
        let mut state = CombatState::initial_fixture();
        state.player.authority.relics = vec![Relic::DeadBranch, Relic::LetterOpener];
        state.monsters = (0..4)
            .map(|i| {
                let mut m = monster_state_for_ascension(
    ...
    fn havoc_played_burning_pact_defers_charons_ashes_until_confirm() {
        ...
        state.player.energy = 1;
        state.player.authority.relics = vec![Relic::CharonsAshes];
        state.piles.hand = vec![
            CardInstance::new(CardId::new(1), HAVOC_ID),
            CardInstance::new(CardId::new(2), STRIKE_R_ID),
            CardInstance::new(CardId::new(3), DEFEND_R_ID),
    ...
    fn charons_ashes_breaking_block_applies_hand_drill_vulnerable() {
        ...
        let mut state = CombatState::cultist_fixture();
        state.player.authority.relics = vec![Relic::CharonsAshes, Relic::HandDrill];
        state.monsters[0].block = 3;
    ...
    fn havoc_lethal_strike_applies_burning_blood_once() {
        ...
        let mut state = CombatState::initial_fixture();
        state.player.authority.relics = vec![Relic::BurningBlood];
        state.player.hp = 50;
    ...
    fn havoc_letter_opener_resolves_before_play_top_malleable_block() {
        ...
        state.monsters[0].powers.malleable_base = 3;
        state.player.energy = 1;
        state.player.authority.relics.push(Relic::LetterOpener);
    ...
    fn havoc_play_top_feel_no_pain_applies_after_source_exhaust_under_corruption() {
        ...
        state
            ...
            .authority
            .relics
            .push(crate::relic::Relic::DeadBranch);
        state.piles.hand = vec![CardInstance::new(CardId::new(1), HAVOC_ID)];
