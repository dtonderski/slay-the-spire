...
pub(super) fn play_card_queue(
    ...
    let queue = play_card_queue_in_place(&mut queued_state, card_id, target)?;
    Ok((queued_state, queue))
}

pub(super) fn play_card_queue_in_place(
    ...
    let queue = match definition.id {
        _ if definition.keywords.unplayable => unplayable_relic_queue(
        ...
        INFERNAL_BLADE_ID | INFERNAL_BLADE_PLUS_ID => {
            infernal_blade_queue(state, card_id, definition)
        }
        CHRYSALIS_ID | CHRYSALIS_PLUS_ID => chrysalis_queue(state, card_id, definition),
        METAMORPHOSIS_ID | METAMORPHOSIS_PLUS_ID => metamorphosis_queue(state, card_id, definition),
        DISCOVERY_ID | DISCOVERY_PLUS_ID => discovery_queue(state, card_id, definition),
        JACK_OF_ALL_TRADES_ID | JACK_OF_ALL_TRADES_PLUS_ID => {
            jack_of_all_trades_queue(state, card_id, definition)
        }
        ...
        CHARGE_BATTERY_ANY_COLOR_ID => {
            state.player.energy_next_turn = state
                .player
                .energy_next_turn
                .checked_add(1)
        ...
        EQUILIBRIUM_ANY_COLOR_ID => {
            state.player.retain_hand_next_turn = true;
    ...
    apply_strange_spoon_to_played_card_move(state, definition, card_id, &mut queue);
...
fn should_apply_necronomicon(
    ...
    if definition.card_type != CardType::Attack
        || !state.player.authority.relics.contains(&Relic::Necronomicon)
...
fn apply_strange_spoon_to_played_card_move(
    ...
    if definition.card_type == CardType::Power
        || !state.player.authority.relics.contains(&Relic::StrangeSpoon)
    {
        return;
    }
...
fn attack_damage_with_strike_dummy(
    ...
    Ok(if is_strike_named_definition(definition) {
        strike_damage_with_relics(&state.player.authority.relics, damage)
    } else {
        damage
    })
}
    ...
    Ok(VecDeque::from([
        ...
        InternalAction::DealDamage {
            info: DamageInfo {
                source: DamageSource::Card(card_id),
                target,
                amount: strike_damage_with_relics(&state.player.authority.relics, base_damage),
            },
        },
        InternalAction::MoveCard {
            card_id,
...
fn malaise_queue(
    ...
    if state.player.authority.relics.contains(&Relic::ChemicalX) {
...
fn whirlwind_queue(
    ...
    let chemical_x_bonus = if state.player.authority.relics.contains(&Relic::ChemicalX) {
        CHEMICAL_X_BONUS_X
    } else {
        0
    };
