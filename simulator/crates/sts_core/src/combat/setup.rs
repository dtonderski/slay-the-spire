#[cfg(test)]
use crate::content::cards::is_basic_starter_card;
use crate::{
    card::CardInstance,
    combat::CardPiles,
...
#[must_use]
fn order_deck_for_combat_shuffle(deck: &[CardInstance]) -> Vec<CardInstance> {
    deck.to_vec()
}
...
#[cfg(test)]
fn starter_only_deck(deck: &[CardInstance]) -> SimResult<bool> {
    for card in deck {
