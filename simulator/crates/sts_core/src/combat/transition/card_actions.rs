...
pub(super) fn spend_energy(state: &mut CombatState, amount: i32) -> SimResult<Vec<InternalAction>> {
    if amount == 0 {
        if let Some(card_id) = state.card_in_use {
            if let Some(card) = state.piles.hand.iter_mut().find(|card| card.id == card_id) {
...
pub(super) fn spend_card_energy(
    ...
    let free_to_play_once = card.free_to_play_once;
    let cost = if free_to_play_once {
        0
    } else {
