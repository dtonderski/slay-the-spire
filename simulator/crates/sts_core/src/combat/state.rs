use crate::{
    ...
    relic::{
        Relic, RelicCounters, HAPPY_FLOWER_THRESHOLD, INCENSE_BURNER_THRESHOLD,
        INK_BOTTLE_THRESHOLD, NUNCHAKU_THRESHOLD, PEN_NIB_THRESHOLD,
    },
    rng::{rng_counter_is_supported, RngTraceStream, StsRng},
    snapshot::{Snapshot, SNAPSHOT_SCHEMA_VERSION},
...
/// Player data that has exactly one live owner: either a run outside combat or
/// the active combat. Keeping these fields together lets combat fixtures remain
/// standalone while integrated runs move the value at phase boundaries.
...
pub struct PlayerState {
    #[serde(flatten)]
    pub authority: PlayerAuthorityState,
    pub block: i32,
    pub energy: i32,
    ...
    /// FrailPower.atEndOfRound skips one decrement after a monster apply.
    #[serde(default, skip_serializing_if = "is_false")]
    pub frail_just_applied: bool,
}

impl std::ops::Deref for PlayerState {
...
impl std::ops::DerefMut for PlayerState {
...
impl PlayerState {
    pub(crate) fn new_run_entry(hp: i32, max_hp: i32, energy_per_turn: i32) -> SimResult<Self> {
        ...
        Ok(Self {
            authority: PlayerAuthorityState {
                hp,
                max_hp,
                relics: Vec::new(),
...
impl std::ops::Deref for CombatState {
    ...
    fn deref(&self) -> &Self::Target {
        &self.player.authority
    }
}

impl std::ops::DerefMut for CombatState {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.player.authority
    }
}
...
impl CombatState {
    ...
    pub(crate) fn from_entry_parts(
        ...
        player.authority.relics = relics;
        Self {
            player,
            monsters,
            piles,
    ...
    pub fn validate(&self) -> SimResult<()> {
        ...
        if self.relic_counters.has_out_of_bounds_stable_counter()
            ...
            || self.sundial_shuffles > i32::MAX as u32
        {
            return Err(SimError::InvalidState(
...
mod tests {
    ...
    fn combat_validation_rejects_consumed_relic_counter_values() {
        ...
        state.ink_bottle_cards_played = crate::relic::INK_BOTTLE_THRESHOLD;
        assert_eq!(
            state.validate(),
            Err(SimError::InvalidState(
                "combat relic counter is outside its stable range"
