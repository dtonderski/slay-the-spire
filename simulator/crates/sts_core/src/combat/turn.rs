...
pub fn end_player_turn(state: &CombatState) -> SimResult<CombatState> {
    end_player_turn_owned(state.clone())
}
...
pub(crate) fn end_player_turn_owned(mut next: CombatState) -> SimResult<CombatState> {
    ...
    if pre_discard_settled {
        ...
        let stone_calendar_would_finish = next
            .player
            .authority
            .relics
            .contains(&crate::relic::Relic::StoneCalendar)
        ...
        if next
            ...
            .authority
            .relics
            .contains(&crate::relic::Relic::NilrysCodex)
...
fn apply_pending_player_spikes_damage(state: &mut CombatState) -> SimResult<()> {
    ...
    let hp_loss =
        crate::combat::damage::reflect_spikes_to_player(&mut state.player, &relics, damage);
...
fn start_player_turn_in_place(
    ...
    if crate::relic::preserves_energy_between_turns(&state.player.authority.relics) {
...
fn player_can_revive_after_monster_hit(state: &CombatState) -> bool {
    ...
    (state
        ...
        .authority
        .relics
        .contains(&crate::Relic::LizardTail)
        && !state.lizard_tail_used)
    ...
    match state.monsters[index].intent {
            ...
            if is_half_dead_darkling(&state.monsters[index]) =>
        {
            ...
            if state
                ...
                .authority
                .relics
                .contains(&crate::Relic::PhilosophersStone)
            {
...
fn execute_spawning_or_targeted_special_intent(
    ...
    match state.monsters[index].intent {
        ...
        crate::MonsterIntent::SummonGremlins { count } => {
            ...
            apply_spawn_relic_effects(
                &mut state.monsters,
                max_existing_monster_id,
                &state.player.authority.relics,
            )?;
...
fn apply_monster_pending_effects(
    ...
    let relics = state.relics.clone();
    apply_queued_post_attack_player_debuffs(intent, &mut state.player, &relics)?;
    ...
    if weak > 0 {
        let had_no_weak = state.player.powers.weak == 0;
        crate::relic::apply_player_weak_with_relics(&mut state.player.powers, &relics, weak)?;
        if had_no_weak && state.player.powers.weak > 0 {
            state.player.weak_just_applied = true;
        }
    }
...
pub(crate) fn target_hand_size(state: &CombatState) -> usize {
    HAND_SIZE
        + if state
            ...
            .authority
            .relics
            .contains(&crate::Relic::SneckoEye)
        {
...
mod tests {
    ...
    fn orichalcum_checks_zero_block_before_metallicize_resolves() {
        ...
        state.player.authority.relics = vec![Relic::Orichalcum];
    ...
    fn runic_pyramid_end_holds_pending_hidden_cards() {
        let mut state = CombatState::initial_fixture();
        state.player.authority.relics = vec![Relic::RunicPyramid];
    ...
    fn independent_start_turn_block_relics_each_trigger_juggernaut() {
        ...
        state.player.authority.relics = vec![Relic::HornCleat, Relic::SelfFormingClay];
    ...
    fn juggernaut_triggers_for_orichalcum_metallicize_and_self_forming_clay() {
        ...
        state.player.powers.juggernaut = 7;
        state.player.powers.metallicize = 3;
        state.player.authority.relics = vec![Relic::Orichalcum, Relic::SelfFormingClay];
        state.monsters = vec![monster_state_for_ascension(
            &GIANT_HEAD_A0,
            MonsterId::new(1),
            state.ascension,
    ...
    fn mercury_stasis_release_runs_after_warped_tongs() {
        ...
        state.player.authority.relics = vec![Relic::MercuryHourglass, Relic::WarpedTongs];
        state.piles.hand.clear();
    ...
    fn combust_lethal_applies_constricted_before_combust_lose_hp() {
        ...
        state.piles.hand = vec![CardInstance::new(CardId::new(1), STRIKE_R_ID)];
        state.player.authority.relics.clear(); // no Burning Blood
    ...
    fn end_turn_blocked_burn_does_not_trigger_rupture_before_constricted() {
        ...
        state.piles.draw_pile = (2..=12)
            ...
            .collect();
        state.player.authority.relics.clear();
    ...
    fn stone_calendar_respects_block_when_predicting_finish() {
        ...
        state.player.authority.relics = vec![Relic::BurningBlood, Relic::StoneCalendar];
    ...
    fn fairy_revives_before_lizard_tail_when_both_are_available() {
        ...
        state.player.authority.relics.push(Relic::LizardTail);
        state.lizard_tail_used = false;
        state.relic_counters.fairy_heal_percent = 30;
        let expected_hp =
            revival_hp_with_relics(state.player.max_hp, 30, &state.player.authority.relics)
    ...
    fn centennial_puzzle_draws_before_attack_generated_slimed_enters_discard() {
        let mut state = CombatState::initial_fixture();
        state.player.authority.relics = vec![Relic::CentennialPuzzle];
    ...
    fn book_of_stabbing_multi_hit_defers_runic_cube_so_abacus_cannot_block_later_hits() {
        ...
        state.player.max_hp = 10000;
        state.player.block = 0;
        state.player.authority.relics = vec![Relic::RunicCube, Relic::TheAbacus];
        state.monsters = vec![monster_state_for_ascension(
            &BOOK_OF_STABBING_A0,
            actor_id,
            0,
    ...
    fn lethal_multi_hit_preserves_only_pre_lethal_runic_cube_draws() {
        ...
        state.player.block = 0;
        state.player.authority.relics = vec![Relic::RunicCube];
        state.monsters = vec![monster_state_for_ascension(
            &BOOK_OF_STABBING_A0,
            actor_id,
            0,
    ...
    fn darkling_fire_breathing_from_deferred_cube_draw_restores_count() {
        ...
        state.player.authority.relics.push(Relic::RunicCube);
        state.piles.draw_pile = vec![
    ...
    fn half_dead_darkling_reincarnates_then_rolls_next_move() {
        ...
        state.monsters[1].intent = crate::MonsterIntent::Block { block: 12 };
        state.monsters[2].intent = crate::MonsterIntent::Block { block: 12 };
        state.player.authority.relics.push(Relic::PhilosophersStone);
    ...
    fn nilry_resume_exhausts_unplayed_ethereal_before_discard() {
        ...
        let mut state = CombatState::initial_fixture();
        state.player.authority.relics = vec![Relic::NilrysCodex];
        state.piles.hand = vec![
            CardInstance::new(CardId::new(1), STRIKE_R_ID),
    ...
    fn stacked_mayhem_plays_both_tops_before_ink_bottle_draw() {
        ...
        state.player.authority.relics = vec![crate::Relic::InkBottle];
        state.ink_bottle_cards_played = crate::relic::INK_BOTTLE_THRESHOLD - 1;
        state.piles.hand.clear();
