...
use crate::content::cards::{
    ANGER_ID, APOTHEOSIS_ID, ARMAMENTS_ID, BANDAGE_UP_ID, BARRICADE_ID, BATTLE_TRANCE_ID,
    BERSERK_ID, BLIND_ID, BLOODLETTING_ID, BLOOD_FOR_BLOOD_ID, BLUDGEON_ID, BODY_SLAM_ID,
    BRUTALITY_ID, BURNING_PACT_ID, CARNAGE_ID, CHRYSALIS_ID, CLASH_ID, CLEAVE_ID, CLOTHESLINE_ID,
    ...
    MAGNETISM_ID, MASTER_OF_STRATEGY_ID, MAYHEM_ID, METALLICIZE_ID, METAMORPHOSIS_ID,
...
mod tests {
    ...
    use crate::content::cards::{
        BANDAGE_UP_ID, BLIND_ID, CHRYSALIS_ID, DARK_SHACKLES_ID, DEEP_BREATH_ID, DISCOVERY_ID,
        ENLIGHTENMENT_ID, FLASH_OF_STEEL_ID, FORETHOUGHT_ID, GOOD_INSTINCTS_ID, IMPATIENCE_ID,
        JACK_OF_ALL_TRADES_ID, MAGNETISM_ID, MASTER_OF_STRATEGY_ID, METAMORPHOSIS_ID,
        MIND_BLAST_ID, PANACEA_ID, PANIC_BUTTON_ID, SADISTIC_NATURE_ID, SECRET_TECHNIQUE_ID,
    ...
    #[test]
    fn chrysalis_pool_key_maps_to_concrete_colorless_rare_skill() {
        assert_eq!(shop_card_content_id("CHRYSALIS"), CHRYSALIS_ID);
        assert!(shop_card_is_colorless(CHRYSALIS_ID));
        assert_eq!(shop_card_type(CHRYSALIS_ID), Some(CardType::Skill));
        assert_eq!(shop_card_price_rarity(CHRYSALIS_ID), CardRarity::Rare);
        assert!(colorless_discovery_pool().contains(&CHRYSALIS_ID));
    }

    #[test]
    fn sadistic_nature_pool_key_maps_to_concrete_colorless_power() {
        assert_eq!(shop_card_content_id("SADISTIC_NATURE"), SADISTIC_NATURE_ID);
        assert!(shop_card_is_colorless(SADISTIC_NATURE_ID));
        assert_eq!(shop_card_type(SADISTIC_NATURE_ID), Some(CardType::Power));
        assert_eq!(shop_card_price_rarity(SADISTIC_NATURE_ID), CardRarity::Rare);
        assert!(colorless_discovery_pool().contains(&SADISTIC_NATURE_ID));
    }

    #[test]
    fn panic_button_pool_key_maps_to_concrete_colorless_skill() {
        assert_eq!(shop_card_content_id("PANIC_BUTTON"), PANIC_BUTTON_ID);
        assert!(shop_card_is_colorless(PANIC_BUTTON_ID));
        assert_eq!(shop_card_type(PANIC_BUTTON_ID), Some(CardType::Skill));
        assert_eq!(
            shop_card_price_rarity(PANIC_BUTTON_ID),
            CardRarity::Uncommon
        );
    }

    #[test]
    fn magnetism_pool_key_maps_to_concrete_colorless_power() {
        assert_eq!(shop_card_content_id("MAGNETISM"), MAGNETISM_ID);
        assert!(shop_card_is_colorless(MAGNETISM_ID));
        assert_eq!(shop_card_type(MAGNETISM_ID), Some(CardType::Power));
        assert_eq!(shop_card_price_rarity(MAGNETISM_ID), CardRarity::Rare);
        assert!(colorless_discovery_pool().contains(&MAGNETISM_ID));
    }
    ...
    #[test]
    fn mind_blast_pool_key_maps_to_concrete_colorless_attack() {
        assert_eq!(shop_card_content_id("MIND_BLAST"), MIND_BLAST_ID);
    ...
    #[test]
    fn the_bomb_pool_key_maps_to_concrete_colorless_rare_skill() {
        assert_eq!(shop_card_content_id("THE_BOMB"), THE_BOMB_ID);
    ...
    #[test]
    fn metamorphosis_pool_key_maps_to_concrete_colorless_rare_skill() {
        assert_eq!(shop_card_content_id("METAMORPHOSIS"), METAMORPHOSIS_ID);
        assert!(shop_card_is_colorless(METAMORPHOSIS_ID));
        assert_eq!(shop_card_type(METAMORPHOSIS_ID), Some(CardType::Skill));
        assert_eq!(shop_card_price_rarity(METAMORPHOSIS_ID), CardRarity::Rare);
    }

    #[test]
    fn secret_technique_pool_key_maps_to_concrete_colorless_rare_skill() {
        assert_eq!(
            shop_card_content_id("SECRET_TECHNIQUE"),
            SECRET_TECHNIQUE_ID
        );
        assert!(shop_card_is_colorless(SECRET_TECHNIQUE_ID));
        assert_eq!(shop_card_type(SECRET_TECHNIQUE_ID), Some(CardType::Skill));
        assert_eq!(
