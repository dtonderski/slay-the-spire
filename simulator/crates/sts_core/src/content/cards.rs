...
mod tests {
    ...
    #[test]
    fn deep_breath_has_expected_values_and_rarity() {
        assert_eq!(DEEP_BREATH.id, DEEP_BREATH_ID);
        assert_eq!(DEEP_BREATH.cost, 0);
        assert_eq!(DEEP_BREATH.target, TargetRequirement::None);
        assert_eq!(DEEP_BREATH.card_type, CardType::Skill);
    ...
    #[test]
    fn secret_technique_has_expected_values_keywords_and_rarity() {
        assert_eq!(SECRET_TECHNIQUE.id, SECRET_TECHNIQUE_ID);
        assert_eq!(SECRET_TECHNIQUE.cost, 0);
        assert_eq!(SECRET_TECHNIQUE.target, TargetRequirement::None);
        assert_eq!(SECRET_TECHNIQUE.card_type, CardType::Skill);
        assert_eq!(
            SECRET_TECHNIQUE.keywords,
            CardKeywords {
                innate: false,
                ethereal: false,
                exhaust: true,
    ...
    #[test]
    fn anger_plus_deals_two_more_damage() {
        assert_eq!(ANGER_PLUS.values.damage, Some(8));
    }

    #[test]
    fn cleave_plus_deals_one_more_damage() {
        assert_eq!(CLEAVE_PLUS.values.damage, Some(11));
    }

    #[test]
    fn twin_strike_plus_deals_two_more_damage() {
        assert_eq!(TWIN_STRIKE_PLUS.values.damage, Some(7));
    }
    ...
    fn shrug_it_off_has_expected_values() {
        ...
        assert_eq!(
            upgrade_content_id(SHRUG_IT_OFF_ID),
            Some(SHRUG_IT_OFF_PLUS_ID)
        );
    ...
    #[test]
    fn dual_wield_does_not_exhaust_and_plus_keeps_base_cost() {
        assert!(!DUAL_WIELD.keywords.exhaust);
        assert_eq!(DUAL_WIELD_PLUS.cost, 1);
        assert!(!DUAL_WIELD_PLUS.keywords.exhaust);
    }
