...
pub const PUBLIC_MONSTER_DEFINITIONS: &[&MonsterDefinition] = &[
...
pub fn public_monster_definitions() -> impl Iterator<Item = &'static MonsterDefinition> {
    PUBLIC_MONSTER_DEFINITIONS.iter().copied()
}

#[must_use]
pub fn get_monster_definition(content_id: ContentId) -> Option<&'static MonsterDefinition> {
    public_monster_definitions().find(|definition| definition.content_id == content_id)
}
