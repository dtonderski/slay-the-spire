...
use crate::diff::canonical_value_diff;
use crate::trace::{parse_trace_jsonl_line, TraceAction, TraceLine, TraceProfile, TraceState};
use serde::Serialize;
use serde_json::{json, Value};
use std::{collections::BTreeSet, io::BufRead};
use sts_core::adapter_internals::card::CardType;
use sts_core::adapter_internals::combat::{ExhaustSelectPurpose, HandSelectPurpose};
use sts_core::adapter_internals::content::cards::card_type_and_rarity;
use sts_core::adapter_internals::content::encounters::BossUnlockState;
use sts_core::adapter_internals::content::monsters::{
    target_move_byte, target_move_byte_for_monster,
};
...
fn legal_map_decisions(run: &RunState) -> sts_core::adapter_internals::SimResult<Vec<MapAction>> {
...
fn legal_rest_decisions(run: &RunState) -> sts_core::adapter_internals::SimResult<Vec<RestAction>> {
    Ok(legal_run_decision_actions(run)?
        .into_iter()
        .filter_map(|action| match action {
...
pub struct SimRealReport {
    ...
    pub terminal: bool,
    pub failure: Option<SeedStartBoundary>,
    unexpected_diff: Option<UnexpectedDiff>,
}

#[derive(Debug)]
struct UnexpectedDiff {
    action_step: u32,
    command: String,
    diffs: Vec<String>,
}
...
impl StartRunCommand {
    fn matches_command(&self, command: &str) -> bool {
        command.eq_ignore_ascii_case(&format!(
            "START_VERIFY {} {} {} {}",
            self.character, self.ascension, self.external_seed, self.verification_starting_hp
        ))
    }
}
...
pub enum SimRealError {
    ...
    RejectedCommand { step: u32, command: String },
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UnsupportedSchema { boundary_schema } => match boundary_schema {
                Some(schema) => write!(
                    f,
                    "unsupported CommunicationMod boundary schema {schema}; supported schemas are 6 and 7"
                ),
                None => write!(f, "CommunicationMod boundary schema 6 or 7 is required"),
            },
            ...
            Self::RejectedCommand { step, command } => {
                write!(f, "target rejected command {command:?} at step {step}")
            }
        }
    }
}
...
pub fn verify_communication_mod_trace_reader<R: BufRead>(
    ...
    verify_seed_start_reader(reader)
}

struct PendingStreamingAction {
    action: TraceAction,
    external_rng: Vec<sts_core::adapter_internals::ExternalRngInput>,
}
...
fn verify_seed_start_reader<R: BufRead>(reader: R) -> Result<SimRealReport, SimRealError> {
    ...
    // Temporary verifier-only fallback for traces collected before action-time
    // metadata was recorded. Only the immediately preceding source observation
    // may supply this external timing input; post-action state is never used.
    let mut last_observed_playtime_seconds = None;
    let mut metadata_seen = false;
    let mut boundary_schema = None;
        ...
        if let TraceLine::Metadata(metadata) = line {
            if record_count != 1 || metadata_seen {
                return Err(SimRealError::InvalidBoundaryContract {
                    step: 0,
                    reason: "trace requires exactly one leading metadata record".to_owned(),
                });
            }
            ...
            let schema = metadata
                .boundary_schema
                .ok_or(SimRealError::UnsupportedSchema {
                    boundary_schema: None,
                })?;
            if !matches!(schema, 6 | 7) {
                return Err(SimRealError::UnsupportedSchema {
                    boundary_schema: Some(schema),
                });
            }
            boundary_schema = Some(schema);
            let explicit_profile = metadata
                ...
                .ok_or_else(|| {
                    SimRealError::InvalidProfileInput(
                        "metadata.run_config.profile is required".to_owned(),
                    )
                })?;
        ...
        match line {
            TraceLine::Action(action) => {
                if pending.is_some() {
                    return Err(SimRealError::InvalidBoundaryContract {
                        step: action.step,
                        reason: "action has no immediate completion".to_owned(),
                    });
                }
                let command_id = action_command_id(&action).ok_or_else(|| {
                    SimRealError::InvalidBoundaryContract {
                        step: action.step,
                        reason: "action requires command_meta.command_id".to_owned(),
                    }
                })?;
                if !seen_command_ids.insert(command_id.to_owned()) {
                    return Err(SimRealError::InvalidBoundaryContract {
                        step: action.step,
                        reason: format!("duplicate command_id {command_id:?}"),
                    });
                }
                let expected_step = last_action_step.map_or(1, |step: u32| step + 1);
                if action.step != expected_step {
                    return Err(SimRealError::InvalidBoundaryContract {
                        step: action.step,
                        reason: format!("expected action step {expected_step}"),
                    });
                }
                ...
                if start.is_none() {
                    start = parse_start_command(&action).transpose()?;
                    if start.is_none() {
            ...
            TraceLine::ExternalRng(capture) => {
                let pending = pending
                    .as_mut()
                    .ok_or(SimRealError::OrphanExternalRng { step: capture.step })?;
                if pending.action.step != capture.step {
                    return Err(SimRealError::OrphanExternalRng { step: capture.step });
                }
                pending.external_rng.extend(capture.draws);
            }
            TraceLine::State(state) => {
                let schema = boundary_schema.expect("validated leading metadata");
                let kind = validate_boundary_state(&state, schema)?;
                let pending =
                    pending
                        .take()
                        .ok_or_else(|| SimRealError::InvalidBoundaryContract {
                            step: state.step,
                            reason: "state has no preceding action".to_owned(),
                        })?;
                if state.step != pending.action.step {
                    return Err(SimRealError::InvalidBoundaryContract {
                        step: state.step,
                        reason: format!(
                            "state does not complete action step {}",
                            pending.action.step
                        ),
                    });
                }
                let state_command = command_head_eq(&pending.action.command, "STATE");
                if schema == 7 {
                ...
                last_command_settlement_seq = state
                    ...
                    .and_then(Value::as_u64);
                report.terminal = trace_state_is_terminal(&state);

                let start = start.as_ref().ok_or(SimRealError::MissingStartCommand)?;
                if start.matches_command(&pending.action.command) {
                    saw_start_completion = true;
                }
                report.failure = verify_seed_start_transition(
                    &mut replay,
                    &pending.action,
                    &state,
                    &pending.external_rng,
                    &mut report,
                    SeedStartReplayInputs {
                        start,
                        boss_unlocks,
                        profile: profile.as_ref().expect("validated profile"),
            ...
            TraceLine::Error(error) => {
                let pending = pending.ok_or_else(|| SimRealError::InvalidBoundaryContract {
                    step: error.step,
                    reason: "error has no preceding action".to_owned(),
                })?;
                if pending.action.step != error.step {
                    return Err(SimRealError::InvalidBoundaryContract {
                        step: error.step,
                        reason: "error does not complete the preceding action".to_owned(),
                    });
                }
                return Err(SimRealError::RejectedCommand {
                    step: error.step,
                    command: pending.action.command,
                });
            }
            TraceLine::Metadata(_) => unreachable!(),
        }
    }

    if let Some(pending) = pending {
        return Err(SimRealError::InvalidBoundaryContract {
            step: pending.action.step,
            reason: "action has no completion at end of trace".to_owned(),
        });
    }
    if !metadata_seen || !saw_start_completion {
        return Err(SimRealError::MissingStartCommand);
    }
    if report.failure.is_none() && report.verified_actions != report.total_actions {
        return Err(SimRealError::InvalidBoundaryContract {
            step: last_action_step.unwrap_or(0),
            reason: "not every action produced one verified transition".to_owned(),
        });
    }
    Ok(report)
}

fn validate_boundary_state(state: &TraceState, schema: u32) -> Result<&str, SimRealError> {
    let message = &state.message;
    let state_boundary_schema = message.get("boundary_schema").and_then(Value::as_u64);
    if state_boundary_schema != Some(u64::from(schema)) {
        return Err(SimRealError::InvalidBoundaryContract {
            step: state.step,
            reason: format!(
                "state boundary_schema must be {schema}, received {}",
        ...
        .is_none()
    {
        return Err(SimRealError::InvalidBoundaryContract {
            step: state.step,
            reason: format!("boundary schema {schema} requires boolean end_turn_queued"),
        });
    }
    if message
        .get("command_execution_seq")
        ...
        .is_none()
    {
        return Err(SimRealError::InvalidBoundaryContract {
            step: state.step,
            reason: format!(
                "boundary schema {schema} requires non-negative integer command_execution_seq"
            ),
        });
    }
    for field in [
        ...
        if message.get(field).and_then(Value::as_u64) != Some(0) {
            return Err(SimRealError::InvalidBoundaryContract {
                step: state.step,
                reason: format!("boundary schema {schema} requires {field}=0"),
            });
        }
    }

    if schema == 6 {
        return validate_common_boundary_state(state, message);
    }
    ...
    let response_id = match message.get("command_response_id") {
        ...
        _ => {
            return Err(SimRealError::InvalidBoundaryContract {
                step: state.step,
                reason:
                    "boundary schema 7 requires command_response_id to be a nonempty string or null"
                        .to_owned(),
            });
        }
    };
    ...
    if matches!(response_kind, "settled" | "poll") && (response_id.is_none() || transaction_pending)
    {
        return Err(SimRealError::InvalidBoundaryContract {
            step: state.step,
            reason: format!(
    ...
    validate_common_boundary_state(state, message)
}

fn validate_common_boundary_state<'a>(
    state: &TraceState,
    message: &'a Value,
) -> Result<&'a str, SimRealError> {
    let kind = message
        .get("boundary_kind")
        .and_then(Value::as_str)
        .ok_or_else(|| SimRealError::InvalidBoundaryContract {
...
fn seed_start_take_first_diff_boundary(report: &mut SimRealReport) -> Option<SeedStartBoundary> {
    let diff = report.unexpected_diff.take()?;
    Some(SeedStartBoundary {
        path: format!("$.actions[step={}].command", diff.action_step),
        category: "state_mismatch".to_owned(),
        reason: format!("{}: {}", diff.command, diff.diffs.join("; ")),
    })
}
...
fn parse_start_command(action: &TraceAction) -> Option<Result<StartRunCommand, SimRealError>> {
    ...
    let verification_starting_hp = match parts[4].parse::<i32>() {
        Ok(hp @ 1..=1_000_000) => hp,
        _ => {
            return Some(Err(SimRealError::MalformedStartCommand(
                action.command.clone(),
            )))
...
fn simulated_card_projection_key(card: &CardInstance) -> String {
    ...
    if sts_core::adapter_internals::content::cards::get_card_definition(card.content_id).is_none() {
        if let Some(key) =
            sts_core::adapter_internals::run::reward::any_color_reward_card_key(card.content_id)
        {
...
fn seed_start_rest_screen_actions(
    run: &RunState,
) -> sts_core::adapter_internals::SimResult<Vec<RestAction>> {
...
fn shop_card_trace_label(run: &RunState, card: &CardInstance) -> String {
    ...
    if egg_preview_upgrade(run, card.content_id)
        == Some(sts_core::adapter_internals::content::cards::SEARING_BLOW_PLUS_ID)
    {
...
fn shop_card_display_key(run: &RunState, content_id: ContentId) -> &'static str {
    use sts_core::adapter_internals::content::cards::INFLAME_ID;
...
fn shop_pool_trace_name(content_id: ContentId) -> Option<&'static str> {
    use sts_core::adapter_internals::content::shop_pool::shop_card_content_id;
    const NAMES: &[(&str, &str)] = &[
...
fn grid_trace_choice_label(_run: &RunState, card: &CardInstance) -> String {
    use sts_core::adapter_internals::content::cards::{
    ...
    if sts_core::adapter_internals::content::cards::is_synthetic_any_color_content_id(
        card.content_id,
    ) {
        if let Some(key) =
            sts_core::adapter_internals::run::reward::any_color_reward_card_key(card.content_id)
...
fn seed_start_simulated_map_return(run: &RunState) -> Result<Value, String> {
    ...
    let next_node_ids = legal_actions
        ...
        .map(|action| match action {
            sts_core::adapter_internals::MapAction::ChooseNode { node_id } => node_id,
        })
        .collect::<Vec<_>>();
...
fn seed_start_map_node_xy(node_id: sts_core::adapter_internals::MapNodeId) -> (i32, i64) {
    if node_id.get() == 0 {
        return (0, -1);
    }
...
fn seed_start_trace_monster_name(monster: &MonsterState) -> String {
    use sts_core::adapter_internals::combat::SlimeSize;
    use sts_core::adapter_internals::content::monsters::{
...
fn seed_start_trace_intent(monster: &MonsterState) -> String {
    use sts_core::adapter_internals::content::monsters::{GREEN_LOUSE_ID, RED_LOUSE_ID};
    ...
    cards
        ...
        .map(|card| {
            ...
            if let Some(content_id) = content_id_from_card_value(card) {
                if sts_core::adapter_internals::content::cards::is_synthetic_any_color_content_id(content_id) {
                    if let Some(pool_key) =
                        sts_core::adapter_internals::run::reward::any_color_reward_card_key(content_id)
                    {
                ...
                if sts_core::adapter_internals::content::cards::get_card_definition(content_id).is_none() {
                    if let Some(identity) = identity.as_deref() {
                        if let Some(pool_key) =
                            sts_core::adapter_internals::run::reward::any_color_reward_card_key_from_identity(identity)
                        {
                            return json!(normalize_card_identity(pool_key));
                        }
                    }
...
fn simulated_card_reward_identity(content_id: ContentId) -> Value {
    if !sts_core::adapter_internals::content::cards::is_synthetic_any_color_content_id(content_id)
        && sts_core::adapter_internals::content::cards::get_card_definition(content_id).is_some()
    {
        return json!(content_id.get());
    }
    sts_core::adapter_internals::run::reward::any_color_reward_card_key(content_id)
...
fn seed_start_event_simulated_subset_with_deck(run: &RunState, deck_ids: Vec<String>) -> Value {
    ...
    let event_id = run
        ...
        .map(|event| match event.event {
            sts_core::adapter_internals::Event::Neow => "neowevent".to_owned(),
            sts_core::adapter_internals::Event::TheSsssserpent => "liarsgame".to_owned(),
            sts_core::adapter_internals::Event::HypnotizingColoredMushrooms => {
                "mushrooms".to_owned()
            }
            // CommunicationMod id is `The Moai Head` → themoaihead (FIDL00232).
            sts_core::adapter_internals::Event::MoaiHead => "themoaihead".to_owned(),
...
fn seed_start_communication_mod_card_id(content_id: ContentId) -> String {
    let source_card_id =
        sts_core::adapter_internals::content::cards::communication_mod_card_id(content_id)
...
fn sole_living_enemy_target_if_required(
    ...
    use sts_core::adapter_internals::card::TargetRequirement;
    use sts_core::adapter_internals::content::cards::get_card_definition;
...
fn compare_subset(
    ...
    if diffs.is_empty() {
        report.verified_actions += 1;
    } else {
        report.unexpected_diff = Some(UnexpectedDiff {
...
fn content_id_from_card_value(card: &Value) -> Option<ContentId> {
    ...
    if sts_core::adapter_internals::content::cards::is_synthetic_any_color_content_id(base) {
        return Some(base);
    }
...
fn card_content_id_is_upgraded(content_id: ContentId) -> bool {
    sts_core::adapter_internals::content::cards::ALL_CARDS
        .iter()
...
fn observed_card_projection_key(card: &Value) -> Option<String> {
    let Some(content_id) = content_id_from_card_value(card) else {
        let identity = observed_display_card_identity(card)?;
        if let Some(pool_key) =
            sts_core::adapter_internals::run::reward::any_color_reward_card_key_from_identity(
                &identity,
            )
        {
...
fn upgrade_content_id(base: ContentId) -> Option<ContentId> {
    sts_core::adapter_internals::content::cards::upgrade_content_id(base)
}
...
fn content_id_from_key(key: &str) -> Option<ContentId> {
    ...
    if let Some(definition) = sts_core::adapter_internals::content::cards::ALL_CARDS
        .iter()
        .find(|definition| {
    ...
    match key {
        ...
        "ClearTheMind" | "clear the mind" => {
            ...
            .map(sts_core::adapter_internals::content::shop_pool::shop_card_content_id)
        }
        ...
        _ => sts_core::adapter_internals::run::reward::any_color_reward_card_key_from_identity(key)
            .map(sts_core::adapter_internals::content::shop_pool::shop_card_content_id),
    }
}
...
fn content_key(content_id: ContentId) -> &'static str {
    if let Some(definition) =
        sts_core::adapter_internals::content::cards::get_card_definition(content_id)
    {
...
fn reward_card_display_key(run: &RunState, content_id: ContentId) -> &'static str {
    use sts_core::adapter_internals::content::cards::WARCRY_PLUS_ID;
...
fn egg_preview_upgrade(run: &RunState, content_id: ContentId) -> Option<ContentId> {
    let upgraded = upgrade_content_id(content_id).or_else(|| {
        sts_core::adapter_internals::content::cards::is_synthetic_any_color_content_id(content_id)
...
fn validate_schema6_completion(
    action: &TraceAction,
    message: &Value,
    kind: &str,
    state_command: bool,
    ...
    let source_execution = action_source_command_execution_seq(action).ok_or_else(|| {
        SimRealError::InvalidBoundaryContract {
            step: action.step,
            reason: "schema-6 action requires command_meta.source_command_execution_seq".to_owned(),
        }
    })?;
    ...
    let execution_seq = message
        .get("command_execution_seq")
        .and_then(Value::as_u64)
        .expect("schema-6 sequence validated");
    if state_command {
        if kind != "poll" || execution_seq != source_execution {
            return Err(SimRealError::InvalidBoundaryContract {
                step: action.step,
                reason: format!(
                    "schema-6 STATE must complete as a poll without advancing execution sequence {source_execution}"
                ),
            });
        }
        return Ok(());
    ...
    if !matches!(kind, "interaction_ready" | "quiescent" | "terminal") {
        return Err(SimRealError::InvalidBoundaryContract {
            step: action.step,
            reason: format!("schema-6 gameplay action completed at unsupported boundary {kind}"),
        });
    }
    let expected_execution =
        source_execution
            ...
            .ok_or_else(|| SimRealError::InvalidBoundaryContract {
                step: action.step,
                reason: "schema-6 source command execution sequence overflow".to_owned(),
            })?;
    if execution_seq != expected_execution {
        return Err(SimRealError::InvalidBoundaryContract {
            step: action.step,
            reason: format!(
                "schema-6 gameplay execution sequence must advance exactly once ({source_execution} -> {expected_execution})"
            ),
        });
    }
    Ok(())
...
fn intent_key(monster: &MonsterState) -> String {
    ...
    if monster.content_id == sts_core::adapter_internals::content::monsters::TIME_EATER_ID {
