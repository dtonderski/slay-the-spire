...
- Full captured topology/edge validation: comparing `CODEX04`'s first CommunicationMod map payload against `generate_exordium_map_topology(22079335079)` now matches every visible node coordinate and outgoing child coordinate. The translated generator deduplicates edges by destination, matching the target map payload and avoiding duplicate children in projected maps.
...
- Extend map parity from full CODEX04 topology/edge parity, fixed rows, desired room counts, shuffled room-list order, CODEX04 captured placement prefix, and `FixedMap` traversal into full room-symbol placement and later reachable node choices.
