#!/usr/bin/env bash
# Per-boot runtime step. Download the current reviewed permanent corpus when
# the runtime-only Hugging Face read token is available.
set -euo pipefail
...
if [[ -n "${HF_TOKEN:-}" ]]; then
  STS_PERMANENT_CORPUS_DIR="$active_corpus" \
    simulator/tools/hf_corpus.sh download dtonderski/sts-permanent-traces
