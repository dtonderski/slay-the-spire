...
from sts_sim.rl.tracking import OfflineWandbConfig, OfflineWandbSession, start_offline_wandb
...
class _FailingSummary(dict[str, object]):
    def __setitem__(self, key: str, value: object) -> None:
        raise RuntimeError("injected wandb.summary failure")
...
class _FakeWandb:
    ...
    def init(self, **kwargs: object) -> _FakeRun:
        ...
        if self.fail_summary:
            self.run.summary = _FailingSummary()
        return self.run
    ...
    def finish(self) -> None:
        self.finish_calls += 1
        if self.fail_finish:
            raise RuntimeError("injected wandb.finish failure")
...
def _assert_rng_equal(
    ...
    numpy_left: object,
    numpy_right: object,
    torch_left: torch.Tensor,
    torch_right: torch.Tensor,
) -> None:
    _assert_nested_equal(python_left, python_right)
    _assert_nested_equal(numpy_left, numpy_right)
    assert torch.equal(torch_left, torch_right)


def test_missing_tracking_extra_fails_closed(
...
def test_offline_session_refuses_nested_tensors(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    _install_fake_wandb(monkeypatch)
    with pytest.raises(TypeError, match="refuses tensors"):
        start_offline_wandb(
            OfflineWandbConfig(project="demo", directory=tmp_path / "wandb"),
            {"items": ({"t": torch.zeros(1)},)},
        )
    session = start_offline_wandb(
        OfflineWandbConfig(project="demo", directory=tmp_path / "nested-wandb"),
        {"trainer": "beam_clone"},
    )
    with pytest.raises(TypeError, match="refuses tensors"):
        session.log_summary({"nested": {"arr": [1, (torch.tensor(1.0),)]}})
...
def test_init_cleanup_restores_env_if_finish_raises(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    fake = _install_fake_wandb(monkeypatch)
    fake.fail_finish = True
    monkeypatch.setenv("WANDB_MODE", "online")
    monkeypatch.setenv("WANDB_SILENT", "false")

    def boom(*_args: object, **_kwargs: object) -> OfflineWandbSession:
        raise RuntimeError("injected session construct failure")

    monkeypatch.setattr("sts_sim.rl.tracking.OfflineWandbSession", boom)
    with (
        pytest.warns(RuntimeWarning, match="init cleanup finish failed"),
        pytest.raises(RuntimeError, match="injected session construct failure"),
    ):
        start_offline_wandb(
            OfflineWandbConfig(project="demo", directory=tmp_path / "wandb"),
            {"trainer": "beam_clone"},
    ...
    assert os.environ.get("WANDB_SILENT") == "false"
    assert fake.finish_calls == 1
    ...
    def fake_train(
        ...
        *,
        ...
        return TrainingResult(
            checkpoint, 1, ({"step": 1, "loss": 0.0},), "a" * 64, "b" * 64, "c" * 64
        )
...
def test_log_failure_after_init_preserves_training_and_checkpoint(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    fake = _install_fake_wandb(monkeypatch)
    fake.fail_log = True
    manifest = _tiny_train_manifest(tmp_path)
    checkpoint = tmp_path / "checkpoint.pt"
    with pytest.warns(RuntimeWarning, match="offline W&B log failed after successful init"):
...
def test_summary_failure_after_init_preserves_training(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    fake = _install_fake_wandb(monkeypatch)
    fake.fail_summary = True
    manifest = _tiny_train_manifest(tmp_path)
    checkpoint = tmp_path / "checkpoint.pt"
    with pytest.warns(RuntimeWarning, match="offline W&B summary failed after successful init"):
        result = train_beam_clone(
            manifest,
            checkpoint,
            _smoke_training_config(),
...
def test_finish_failure_does_not_mask_training_error(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    fake = _install_fake_wandb(monkeypatch)
    fake.fail_finish = True
    ...
    monkeypatch.setattr("sts_sim.rl.training.policy_value_loss", boom)
    with (
        pytest.warns(RuntimeWarning, match="offline W&B finish failed after successful init"),
        pytest.raises(RuntimeError, match="injected training failure"),
    ):
        train_beam_clone(
            manifest,
            tmp_path / "checkpoint.pt",
...
def test_finish_failure_after_success_preserves_training(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    fake = _install_fake_wandb(monkeypatch)
    fake.fail_finish = True
    manifest = _tiny_train_manifest(tmp_path)
    checkpoint = tmp_path / "checkpoint.pt"
    with pytest.warns(RuntimeWarning, match="offline W&B finish failed after successful init"):
        result = train_beam_clone(
            manifest,
            checkpoint,
            _smoke_training_config(),
...
def test_tracked_and_untracked_training_preserve_checkpoint_and_rng_state(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    fake = _install_fake_wandb(monkeypatch)
    manifest = _tiny_train_manifest(tmp_path)
    ...
    after_tracked = (random.getstate(), np.random.get_state(), torch.get_rng_state())
    _assert_rng_equal(
        after_untracked[0],
        after_tracked[0],
        after_untracked[1],
...
def _block_ip_connections(monkeypatch: pytest.MonkeyPatch) -> None:
    original_connect = socket.socket.connect
    original_connect_ex = socket.socket.connect_ex

    def _reject_ip(family: int, action: str, target: object) -> None:
        if family in (socket.AF_INET, socket.AF_INET6):
            raise AssertionError(f"offline W&B smoke blocked IP {action}: {target}")

    def guarded_connect(self: socket.socket, address: tuple[object, ...] | str, /) -> None:
        _reject_ip(self.family, "connect", address)
        return original_connect(self, address)

    def guarded_connect_ex(self: socket.socket, address: tuple[object, ...] | str, /) -> int:
        _reject_ip(self.family, "connect_ex", address)
    ...
    def guarded_create_connection(
        ...
        **kwargs: object,
        ...
        raise AssertionError(f"offline W&B smoke blocked create_connection: {args!r} {kwargs!r}")
...
def test_real_wandb_offline_smoke(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    pytest.importorskip("wandb")
    _block_ip_connections(monkeypatch)
    monkeypatch.setenv("WANDB_MODE", "online")
    monkeypatch.setenv("WANDB_SILENT", "false")
    random.seed(3)
    np.random.seed(3)
    ...
    session = start_offline_wandb(
        OfflineWandbConfig(
            project="sts-combat-smoke", directory=tmp_path / "wandb", run_name="smoke"
        ),
        {"trainer": "beam_clone", "puct_targets_in_training": False},
    )
    session.log_scalars({"loss": 0.25}, step=1)
    session.log_summary({"global_step": 1})
    session.finish()
    _assert_rng_equal(
        before_python,
        random.getstate(),
    ...
    assert os.environ.get("WANDB_MODE") == "online"
    assert os.environ.get("WANDB_SILENT") == "false"
    offline_runs = list((tmp_path / "wandb").glob("wandb/offline-run-*"))
    assert offline_runs
    assert any(path.suffix == ".wandb" for path in offline_runs[0].glob("*.wandb"))
