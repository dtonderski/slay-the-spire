...
### Combat
- **47 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break)
- Latest slice: base Limit Break (`ContentId(141)`, `LIMIT_BREAK`) is implemented as an Ironclad rare skill with cost 1, no target, exhaust, and doubles current player Strength through existing signed Strength semantics. Caveats: upgraded Limit Break is not implemented and there is no played-card CommunicationMod trace parity claim.
