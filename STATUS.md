...
### Combat
- **51 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper)
- Latest slice: base Reaper (`ContentId(149)`, `REAPER`) is implemented as an Ironclad rare attack with cost 2, target all enemies, 4 damage to each living enemy, healing by summed unblocked HP damage, and exhausts. Caveats: upgraded Reaper is not implemented, overkill healing is not separately parity-proven, and there is no played-card CommunicationMod trace parity or source-backed exact action-order claim.
...
- Latest Reaper slice checks: `cargo fmt`, `cargo test -p sts_core reaper -- --test-threads=1`, `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`, `cargo clippy -p sts_core`, and `cargo test -p sts_core -- --test-threads=1`
