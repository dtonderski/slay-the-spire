...
Current milestone: Milestone 32B, deterministic content completion sweep. The latest local slice implements base Heavy Blade (`HEAVY_BLADE`, ContentId(108)) as a deterministic 2-cost common attack with focused definition, targeted legality, base damage, Strength multiplier, temp-Strength multiplier, discard, Akabeko interaction, and event-log coverage. The M32A card matrix now marks Heavy Blade `implemented` with an explicit caveat that no played-card real-game trace parity is claimed yet. Prior local-test-only slices implemented base Clash (`CLASH`, ContentId(102)), base Body Slam (`BODY_SLAM`, ContentId(101)), base Intimidate (`INTIMIDATE`, ContentId(115)), base Clothesline (`CLOTHESLINE`, ContentId(104)), and base Iron Wave (`IRON_WAVE`, ContentId(100)).
