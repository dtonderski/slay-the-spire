...
### Combat
- **64 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire + base Barricade + base Berserk + base Rampage + base Brutality + base Combust + base Double Tap + base Rupture + base Blood for Blood + base Evolve + base Fire Breathing + base Feed)
- Latest slice: base Feed (`ContentId(140)`, `FEED`) is implemented as an Ironclad rare attack with cost 1, enemy target, 10 damage, and exhaust. Feed uses a Feed-scoped damage action so normal attack modifiers and on-kill relic hooks still apply; when that Feed damage kills a target, the player gains 3 max HP and 3 current HP. Evidence: local bytecode inspection of `FeedAction.class` shows the max-HP award belongs to Feed's damage action and is gated on fatal damage with half-dead/Minion exclusions. Caveats: upgraded Feed is not implemented, simulator state does not model minion/half-dead target exclusions in this slice, current-HP gain follows the simulator's existing max-HP-gain convention, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Feed slice checks: `cargo fmt`; `cargo test -p sts_core feed -- --test-threads=1`; `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`; `cargo clippy -p sts_core`; `cargo test -p sts_core -- --test-threads=1`; `cargo test -p sts_verify -- --test-threads=1`.
