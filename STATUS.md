...
### Combat
- **52 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind)
- Latest slice: base Second Wind (`ContentId(128)`, `SECOND_WIND`) is implemented as an Ironclad uncommon skill with cost 1 and no target, exhausting all non-Attack cards in hand except the source, gaining 5 block per exhausted non-source card, then moving Second Wind to discard. Caveats: upgraded Second Wind is not implemented, unknown-definition hand cards follow the existing Sever Soul skip behavior, and there is no played-card CommunicationMod trace parity or source-backed exact action-order claim.
...
- Latest Second Wind slice checks: `cargo fmt`, `cargo test -p sts_core second_wind -- --test-threads=1`, `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`, `cargo clippy -p sts_core`, and `cargo test -p sts_core -- --test-threads=1`
