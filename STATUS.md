...
Current milestone: Milestone 32B, deterministic content completion sweep. The latest local slice implements base Wild Strike (`WILD_STRIKE`, ContentId(107)) as a deterministic 1-cost common attack with focused definition, targeted legality, base damage, Strength, discard, generated Wound-to-draw-pile, local pile-order, Akabeko interaction, and event-log coverage. The M32A card matrix now marks Wild Strike `implemented` with an explicit caveat that no played-card real-game trace parity or exact Wound draw-pile shuffle-order parity is claimed yet. Prior local-test-only slices implemented base Heavy Blade (`HEAVY_BLADE`, ContentId(108)), base Clash (`CLASH`, ContentId(102)), base Body Slam (`BODY_SLAM`, ContentId(101)), base Intimidate (`INTIMIDATE`, ContentId(115)), base Clothesline (`CLOTHESLINE`, ContentId(104)), and base Iron Wave (`IRON_WAVE`, ContentId(100)).
