...
Current milestone: Milestone 32B, deterministic content completion sweep. The latest local slice implements base Clash (`CLASH`, ContentId(102)) as a deterministic 0-cost common attack with focused definition, attacks-only hand legality, mixed-hand rejection, damage, discard, Akabeko interaction, and event-log coverage. The M32A card matrix now marks Clash `implemented` with an explicit caveat that no played-card real-game trace parity is claimed yet. Prior local-test-only slices implemented base Body Slam (`BODY_SLAM`, ContentId(101)), base Intimidate (`INTIMIDATE`, ContentId(115)), base Clothesline (`CLOTHESLINE`, ContentId(104)), and base Iron Wave (`IRON_WAVE`, ContentId(100)).
