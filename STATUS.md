...
- Latest slice: base Discovery (`ContentId(60080667924456)`, `DISCOVERY`) is implemented as a deterministic colorless uncommon skill with cost 1, target None, exhaust, and a local modeled choose-1-of-3 combat-card reward whose selected card costs 0 for the turn. Caveats: upgraded Discovery is not implemented, generated-choice RNG is local/model-scoped, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Discovery slice checks: `cargo fmt`, `cargo test -p sts_core discovery -- --test-threads=1`, and `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`.
