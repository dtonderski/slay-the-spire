...
### Combat
- **57 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire + base Barricade + base Berserk + base Rampage + base Brutality)
- Latest slice: base Brutality (`ContentId(148)`, `BRUTALITY`) is implemented as an Ironclad rare power with cost 0 and target None. On play it spends effective cost, grants serialized player Brutality power, removes the power card, and uses existing power-play relic hooks. At start of player turn, each stack loses 1 HP through existing HP-loss mitigation/hooks and draws 1 card before normal hand refill. Caveats: upgraded Brutality is not implemented, exact real-game start-turn timing/source is not source-backed, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Brutality slice checks: `cargo fmt`, `cargo test -p sts_core brutality -- --test-threads=1`, `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`, `cargo clippy -p sts_core` (existing 8 warnings), `cargo test -p sts_core -- --test-threads=1`, and `cargo test -p sts_verify -- --test-threads=1`.
