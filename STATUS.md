...
### Combat
- **55 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire + base Barricade + base Berserk)
- Latest slice: base Berserk (`ContentId(145)`, `BERSERK`) is implemented as an Ironclad rare power with cost 0 and no target, applying 2 player Vulnerable, granting serialized start-turn energy power state, and removing the played power card through the existing power-card path. Caveats: upgraded Berserk is not implemented, exact real-game power lifecycle/timing is not source-backed, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Berserk slice checks: `cargo fmt`, `cargo test -p sts_core berserk -- --test-threads=1`, `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`, `cargo clippy -p sts_core` (existing 8 warnings), `cargo test -p sts_core -- --test-threads=1`, `cargo test -p sts_verify -- --test-threads=1`, and `git diff --check`.
