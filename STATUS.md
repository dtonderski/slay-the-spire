...
### Combat
- **68 Ironclad cards plus deterministic colorless slices** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire + base Corruption + base Juggernaut + base Barricade + base Berserk + base Rampage + base Brutality + base Combust + base Double Tap + base Rupture + base Blood for Blood + base Evolve + base Fire Breathing + base Feed + base Exhume + base Infernal Blade + base Good Instincts + base Bandage Up + base Finesse + base Flash of Steel + base Blind + base Panacea + base Trip + base Swift Strike + base Dark Shackles)
- Latest slice: base Dark Shackles (`ContentId(18388408013683944583)`, `DARK_SHACKLES`) is implemented as a colorless uncommon Skill with cost 0, target Enemy, exhaust, and temporary -9 monster Strength for the current turn. The existing colorless shop/discovery pool key now maps to the concrete card definition; normal play is legal at zero energy against one living enemy target, spends zero energy, applies a turn-scoped monster Strength reduction, exhausts the source card, reduces the target monster's attack for that monster turn, and restores the temporary Strength reduction during monster-turn cleanup. Caveats: upgraded Dark Shackles is not implemented, monster Artifact interaction is not modeled in local monster powers, and there is no played-card CommunicationMod trace parity claim.
