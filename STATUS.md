...
### Combat
- **48 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering)
- Latest slice: base Offering (`ContentId(151)`, `OFFERING`) is implemented as an Ironclad rare skill with cost 0, no target, exhaust, lose 6 HP through the existing HP-loss mitigation/draw hooks, gain 2 energy, and draw 3 cards. Caveats: upgraded Offering is not implemented and there is no played-card CommunicationMod trace parity claim.
