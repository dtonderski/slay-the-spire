...
### Combat
- **50 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt)
- Latest slice: base Headbutt (`ContentId(106)`, `HEADBUTT`) is implemented as an Ironclad common attack with cost 1, targeted enemy, 9 damage, and an explicit-purpose discard-select flow that puts one selected discard-pile card on top of the draw pile before discarding Headbutt. Caveats: upgraded Headbutt is not implemented, local draw-pile top ordering follows current simulator pile semantics, lethal Headbutt skips discard selection locally, and there is no played-card CommunicationMod trace parity or source-backed exact action-order claim.
