...
- Latest slice: base Deep Breath (`ContentId(57620194214716779)`, `DEEP_BREATH`) is implemented as a deterministic colorless uncommon skill with cost 0, target None, shuffle-discard-into-draw through existing draw semantics, draw 1, and no exhaust by default. The existing colorless shop/discovery pool key now maps to the concrete card definition; normal play spends zero effective energy, draws deterministically, moves to discard, and rejects targets through existing validation. Havoc/top-draw support covers deterministic draw follow-ups through the existing local top-draw queue. Caveats: upgraded Deep Breath is not implemented, Havoc/top-draw ordering follows current simulator pile/queue semantics, and there is no source-backed or played-card CommunicationMod trace parity claim.
