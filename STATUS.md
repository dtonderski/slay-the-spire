...
### Combat
- **53 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire)
- Latest slice: base Fiend Fire (`ContentId(144)`, `FIEND_FIRE`) is implemented as an Ironclad rare attack with cost 2 and an enemy target, exhausting every other current hand card, dealing 7 damage once per exhausted other hand card, then exhausting Fiend Fire through the normal played-card exhaust destination. Caveats: upgraded Fiend Fire is not implemented, exact real-game exhaust/damage ordering is not source-backed, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Fiend Fire slice checks: `cargo fmt`, `cargo test -p sts_core fiend_fire -- --test-threads=1`, `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`, `cargo clippy -p sts_core`, and `cargo test -p sts_core -- --test-threads=1`
