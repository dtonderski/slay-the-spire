...
### Combat
- **65 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire + base Corruption + base Barricade + base Berserk + base Rampage + base Brutality + base Combust + base Double Tap + base Rupture + base Blood for Blood + base Evolve + base Fire Breathing + base Feed)
- Latest slice: base Corruption (`ContentId(142)`, `CORRUPTION`) is implemented as an Ironclad rare power with cost 3 and target None. On play it spends effective cost, grants serialized player Corruption power, removes the power card, and uses existing power-play relic hooks. While active, locally modeled Skills are playable at zero energy and their played-card move is rewritten to exhaust, preserving existing on-exhaust hooks such as Feel No Pain. Caveats: upgraded Corruption is not implemented, exact real-game power lifecycle/timing, Skill cost-modifier precedence, played-Skill exhaust ordering, and Strange Spoon interaction are not source-backed, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Corruption slice checks: `cargo fmt`; `cargo test -p sts_core corruption -- --test-threads=1`; `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`; `cargo clippy -p sts_core`; `cargo test -p sts_core -- --test-threads=1`; `cargo test -p sts_verify -- --test-threads=1`.
