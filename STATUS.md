...
### Combat
- **60 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire + base Barricade + base Berserk + base Rampage + base Brutality + base Combust + base Double Tap + base Rupture)
- Latest slice: base Rupture (`ContentId(129)`, `RUPTURE`) is implemented as an Ironclad uncommon power with cost 1 and target None. On play it spends effective cost, grants serialized player Rupture power, removes the power card, and uses existing power-play relic hooks. Card-authored self HP-loss actions now carry a source tag; actual HP loss from tagged cards grants Strength equal to Rupture stacks, while prevented HP loss and other-source HP loss such as Blue Candle do not trigger it. Caveats: upgraded Rupture is not implemented, exact real-game source/action ordering is not source-backed, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Rupture slice checks: `cargo fmt`; `cargo test -p sts_core rupture -- --test-threads=1`; `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`; `cargo clippy -p sts_core` (passes with existing warnings); `cargo test -p sts_core -- --test-threads=1`; `cargo test -p sts_verify -- --test-threads=1`.
...
Current milestone: Milestone 32B, deterministic content completion sweep. The latest local slice implements base Rupture (`RUPTURE`, ContentId(129)) as a deterministic uncommon Ironclad power: 1 energy, no target, serialized player Rupture power, played-card removal, and source-aware card self-HP-loss Strength gain through the existing HP-loss mitigation/hooks. Coverage includes definition/rarity/cost/target, legality/target rejection, effective-cost spending, serialized power state, event-log ordering, stacking, card-source HP-loss Strength gain before later Hemokinesis damage in the local queue model, no trigger when Buffer prevents actual HP loss, no trigger for Blue Candle other-source HP loss, and Havoc top-draw Offering source propagation. The M32A card matrix now marks Rupture `implemented` with caveats that upgraded Rupture is not implemented, exact real-game source/action ordering is not source-backed, and no played-card real-game trace parity evidence is claimed yet.

Prior local-test-only M32B slices include base Double Tap (`DOUBLE_TAP`, ContentId(137)), base Combust (`COMBUST`, ContentId(123)), base Brutality (`BRUTALITY`, ContentId(148)), Rampage, Berserk, Barricade, Rage, Fiend Fire, Second Wind, Reaper, Headbutt, Armaments, Offering, Limit Break, Shockwave, Dropkick, Disarm, Entrench, Demon Form, Regret, Wound, Dazed, Doubt, Sentinel, Hemokinesis, Carnage, Bloodletting, Ghostly Armor, Pummel, Reckless Charge, Power Through, Perfected Strike, Impervious, Bludgeon, Wild Strike, Heavy Blade, Clash, Body Slam, Intimidate, Clothesline, and Iron Wave.
