...
### Combat
- **54 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire + base Barricade)
- Latest slice: base Barricade (`ContentId(143)`, `BARRICADE`) is implemented as an Ironclad rare power with cost 3 and no target, granting serialized player block-retention power state, removing the played power card through the existing power-card path, and preserving block during turn-transition cleanup. Caveats: upgraded Barricade is not implemented, exact real-game power lifecycle/timing is not source-backed, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Barricade slice checks: `cargo fmt`, `cargo test -p sts_core barricade -- --test-threads=1`, `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`, `cargo clippy -p sts_core`, `cargo test -p sts_core -- --test-threads=1`, `cargo clippy`, and `cargo test`.
