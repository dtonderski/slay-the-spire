...
### Combat
- **59 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire + base Barricade + base Berserk + base Rampage + base Brutality + base Combust + base Double Tap)
- Latest slice: base Double Tap (`ContentId(137)`, `DOUBLE_TAP`) is implemented as an Ironclad rare skill with cost 1 and target None. On play it spends effective cost, adds serialized pending next-Attack replay state, moves to discard, and uses existing Skill-play hooks. The next played Attack consumes the pending state and replays card effect actions once per stack without extra energy or additional played-card movement. Caveats: upgraded Double Tap is not implemented, exact real-game replay timing/source is not source-backed, Duplication Potion/Double Tap combined timing is not source-backed, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Double Tap slice checks: `cargo fmt`; `cargo test -p sts_core double_tap -- --test-threads=1`; `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`; `cargo clippy -p sts_core` (passes with existing warnings); `cargo test -p sts_core -- --test-threads=1`; `cargo test -p sts_verify -- --test-threads=1`.
