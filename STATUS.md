...
### Combat
- **63 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire + base Barricade + base Berserk + base Rampage + base Brutality + base Combust + base Double Tap + base Rupture + base Blood for Blood + base Evolve + base Fire Breathing)
- Latest slice: base Fire Breathing (`ContentId(133)`, `FIRE_BREATHING`) is implemented as an Ironclad uncommon power with cost 1 and target None. On play it spends effective cost, grants serialized player Fire Breathing damage stacks, removes the power card, and uses existing power-play relic hooks. When a Status or Curse is drawn through the shared draw path, each stack deals unmodified all-enemy damage. Caveats: upgraded Fire Breathing is not implemented, exact real-game draw-trigger timing/source is not source-backed, and there is no played-card CommunicationMod trace parity claim.
