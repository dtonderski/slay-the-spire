...
### Combat
- **58 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire + base Barricade + base Berserk + base Rampage + base Brutality + base Combust)
- Latest slice: base Combust (`ContentId(123)`, `COMBUST`) is implemented as an Ironclad uncommon power with cost 1 and target None. On play it spends effective cost, grants serialized player Combust power, removes the power card, and uses existing power-play relic hooks. At end of player turn, each stack loses 1 HP through existing HP-loss mitigation/hooks and deals 5 unmodified damage to all living enemies through local damage hooks. Caveats: upgraded Combust is not implemented, exact real-game end-turn timing/source and stacking semantics are not source-backed, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Combust slice checks: `cargo fmt`; `cargo test -p sts_core combust -- --test-threads=1`; `cargo test -p sts_core combat::hand::tests -- --test-threads=1`; `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`; `cargo clippy -p sts_core` (passes with existing warnings); `cargo test -p sts_core -- --test-threads=1`; `cargo test -p sts_verify -- --test-threads=1`.
