...
- Latest slice: base Impatience (`ContentId(1998026198879085)`, `IMPATIENCE`) is implemented as a colorless uncommon skill with cost 0, target None, draw 2, and no exhaust by default. The existing colorless shop/discovery pool key now maps to the concrete card definition; normal play is legal only when the hand has no other Attack cards, spends zero effective energy, draws deterministically, moves to discard, and rejects targets through existing validation. Havoc/top-draw support covers deterministic draw follow-ups through the existing local top-draw queue. Caveats: upgraded Impatience is not implemented, Havoc top-draw ordering follows current simulator pile/queue semantics, and there is no played-card CommunicationMod trace parity claim.
