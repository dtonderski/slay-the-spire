...
- Latest slice: Milestone 32C evidence backfill corrected source-backed Metamorphosis/Chrysalis generated-card destinations from hand to draw pile, Secret Technique no-skill draw-pile legality, Transmutation's full modeled colorless pool plus temporary zero-cost generation, and Violence's DrawPileToHandAction-style attack selection/hand-full fallback. Milestone 32B deterministic card sweep is integrated, including base Hand Of Greed, Chrysalis, Sadistic Nature, Panic Button, Magnetism, and a verifier fixture update for the new no-block combat state field. Caveats: upgraded colorless cards are not implemented unless separately modeled, many generated-card/random-selection semantics are local deterministic approximations, Metamorphosis/Chrysalis exact random draw-pile insertion position remains local, Hand Of Greed's gold-on-kill payout is not modeled because combat state does not carry run gold, and there is no broad played-card CommunicationMod trace parity claim for these cards yet.
