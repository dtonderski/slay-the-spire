...
### Combat
- **49 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments)
- Latest slice: base Armaments (`ContentId(105)`, `ARMAMENTS`) is implemented as an Ironclad common skill with cost 1, no target, 5 block, and an explicit-purpose hand-select flow that upgrades one non-source hand card with a locally modeled upgrade mapping for the rest of combat. Caveats: upgraded Armaments is not implemented, cards without local `upgrade_content_id` mappings cannot be selected yet, Havoc top-draw Armaments only gains block in the current local model, and there is no played-card CommunicationMod trace parity claim.
