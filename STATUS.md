...
### Combat
- **67 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire + base Corruption + base Juggernaut + base Barricade + base Berserk + base Rampage + base Brutality + base Combust + base Double Tap + base Rupture + base Blood for Blood + base Evolve + base Fire Breathing + base Feed + base Exhume)
- Latest slice: base Exhume (`ContentId(150)`, `EXHUME`) is implemented as an Ironclad rare skill with cost 1, target None, and exhaust. It is playable only when the exhaust pile contains a non-Exhume card, opens an explicit serialized exhaust-pile selection, returns the selected exhausted card to hand, exhausts the source Exhume through existing on-exhaust hooks, spends effective cost, and counts the source exhaust for Dead Branch even though the exhaust pile size is flat after selection. Caveats: upgraded Exhume is not implemented, exact real-game select ordering/source timing is not source-backed, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Exhume slice checks: `cargo fmt`, `cargo test -p sts_core exhume -- --test-threads=1`, `cargo test -p sts_core dead_branch_counts_exhume_source_exhaust_even_when_exhaust_pile_size_is_flat -- --test-threads=1`, `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`, `cargo clippy -p sts_core` (existing warnings may remain), `cargo test -p sts_core -- --test-threads=1`, and `cargo test -p sts_verify -- --test-threads=1`.
