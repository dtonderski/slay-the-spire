...
### Combat
- **61 Ironclad cards** (Milestone 5 complete + Ascender's Bane + Dramatic Entrance + base Flame Barrier + base Limit Break + base Offering + base Armaments + base Headbutt + base Reaper + base Second Wind + base Fiend Fire + base Barricade + base Berserk + base Rampage + base Brutality + base Combust + base Double Tap + base Rupture + base Blood for Blood)
- Latest slice: base Blood for Blood (`ContentId(116)`, `BLOOD_FOR_BLOOD`) is implemented as an Ironclad uncommon attack with cost 4, target Enemy, and 18 damage. It stores a serialized per-card-instance combat-local cost reduction that increases by 1 for each positive player HP-loss event routed through combat HP-loss hooks, including card HP loss, monster damage, spikes reflection, Burn, Regret, and Brutality; prevented HP loss does not reduce the cost. On play it spends the effective reduced cost, deals normal attack damage through the existing damage/relic hooks, and discards. Caveats: upgraded Blood for Blood is not implemented, exact real-game HP-loss trigger semantics/action order are not source-backed, multi-hit/stacked HP-loss granularity follows existing local combat hooks, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Blood for Blood slice checks: `cargo fmt`; `cargo test -p sts_core blood_for_blood -- --test-threads=1`; `cargo test -p sts_core rupture -- --test-threads=1`; `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`; `cargo clippy -p sts_core` (passes with existing 8 warnings); `cargo test -p sts_core -- --test-threads=1`; `cargo test -p sts_verify -- --test-threads=1`.
