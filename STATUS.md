...
Continue Milestone 29 by collecting or trimming a clean `M290001` Sentries run without the bad card-reward loop, then driving seed-start verification past the current step-143 map/event boundary and onward to the Sentries fight. The cleaned single-run prefix can be structurally checked with:

```powershell
node tools\communication\trace_tools.js validate verification\corpus\communication_mod\trace-2026-06-23T02-56-19-245Z.run2.valid-prefix.jsonl
```
