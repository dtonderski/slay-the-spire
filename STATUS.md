...
- Latest slice: base Trip (`ContentId(2584189)`, `TRIP`) is implemented as a colorless uncommon skill with cost 0, target None, and 2 Vulnerable applied to all living enemies. The existing colorless shop/discovery pool key now maps to the concrete card definition; normal play spends zero effective energy, rejects explicit targets through existing no-target validation, applies Vulnerable to each living monster, skips dead monsters, and moves to discard. Caveats: upgraded Trip is not implemented, target-game action-order evidence is not source-backed, and there is no played-card CommunicationMod trace parity claim.
...
- Latest Trip slice checks: `cargo fmt`, `cargo test -p sts_core trip -- --test-threads=1`, `cargo test -p sts_core --test m32a_matrix -- --test-threads=1`, `cargo test -p sts_core -- --test-threads=1`, and `cargo clippy -p sts_core` (existing 8 warnings).
